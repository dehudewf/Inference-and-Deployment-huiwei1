...
Status ArcHunyuanVideo::GetModelRunConfig(ModelRunConfig& model_run_config, const ModelConfig& model_config) {
  model_run_config.position_encoding = PositionEncoding::ROPE;
  model_run_config.layernorm_position = LayerNormPosition::PRE_NORM;
  return Status();
}

Status ArcHunyuanVideo::CreateLayers(LayerCreationContext& creation_context,
...
Status ArcHunyuanVideo::Forward(std::vector<Tensor>& residual_buffer, ForwardingContext& forwarding_context) {
...
/* **************************************
 * ArcHunyuanVideoModel
 */
ArcHunyuanVideoModel::ArcHunyuanVideoModel(const ModelConfig& model_config, const RuntimeConfig& runtime_config,
                                           const int rank, std::shared_ptr<Context> context,
                                           std::shared_ptr<BaseWeight> base_weight)
    : CommonModel(model_config, runtime_config, rank, context) {
  ModelRunConfig model_run_config;
  arc_hunyuan_video_.GetModelRunConfig(model_run_config, model_config);
  CommonModel::InitRunConfig(model_run_config, base_weight);
}

Status ArcHunyuanVideoModel::CreateLayers(LayerCreationContext& creation_context,
                                          ModelCreationConfig& model_creation_config) {
  return arc_hunyuan_video_.CreateLayers(creation_context, model_creation_config);
}

Status ArcHunyuanVideoModel::LayerForward(ForwardingContext& forwarding_context, const RunMode run_mode) {
  std::vector<Tensor>& residual_buffer =
      GetHiddenUnitBuffer(forwarding_context, !forwarding_context.GetContext()->IsChief());
  STATUS_CHECK_RETURN(arc_hunyuan_video_.Forward(residual_buffer, forwarding_context));
