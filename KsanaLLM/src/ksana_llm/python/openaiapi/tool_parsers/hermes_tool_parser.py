...
# Ref:
# https://github.com/vllm-project/vllm/blob/v0.9.1/vllm/entrypoints/openai/tool_parsers/hermes_tool_parser.py
import json
...
class Hermes2ProToolParser(ToolParser):
        ...
        if self.tool_call_start_token not in model_output:
            ...
            try:
                ...
                for match in function_call_tuples:
                    ...
                    try:
                        raw_function_calls.append(json.loads(json_str))
                    except json.JSONDecodeError as e:
                        # 如果解析失败，尝试处理双重大括号的情况
                        if '{{' in json_str:
                            json_str_fixed = json_str.replace('{{', '{').replace('}}', '}')
                            logger.warning(f"Found double braces in tool call JSON, attempt fix: {json_str[:50]}")
                            raw_function_calls.append(json.loads(json_str_fixed))
                        else:
                            raise e
                ...
                logger.exception(
                    "Error in extracting tool call from response: %s", str(e))
                return ExtractedToolCallInformation(tools_called=False,
                                                    tool_calls=[],
                                                    content=model_output)
    ...
    def extract_tool_calls_streaming(
        ...
        try:
            ...
            logger.exception("Error trying to handle streaming tool call: %s", str(e))
