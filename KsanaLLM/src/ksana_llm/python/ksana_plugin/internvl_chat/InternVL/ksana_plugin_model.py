...
try:
    ...
    print(f'FlashAttention2 is not installed: {e}')
...
class InternVLPreprocess:
    ...
    def process_image(self, image_file, input_size=448, max_num=6):
        ...
        transform = self.__build_transform(input_size=input_size)
        images = self.__dynamic_preprocess(image, image_size=input_size, use_thumbnail=True, max_num=max_num)
    ...
    def __dynamic_preprocess(self, image, min_num=1, max_num=6, image_size=448, use_thumbnail=False):
...
class InternVisionModel():
    def __init__(self, model_path):
        ...
        downsample_ratio = self.config.to_dict()['downsample_ratio']
    ...
    def forward(
        ...
        last_hidden_state_result = self.encoder(
            inputs_embeds=hidden_states
        )

        last_hidden_state = last_hidden_state_result
...
class InternAttention(nn.Module):
    ...
    """Quoted from https://huggingface.co/OpenGVLab/InternVL2-26B/blob/main/modeling_intern_vit.py#L177"""
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.embed_dim = config.hidden_size
...
class InternVisionEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = hidden_states + self.drop_path1(self.attn \
        ...
        hidden_states = hidden_states + self.drop_path2(self.mlp(self.norm2(hidden_states) \
                            .to(hidden_states.dtype)) * self.ls2)
