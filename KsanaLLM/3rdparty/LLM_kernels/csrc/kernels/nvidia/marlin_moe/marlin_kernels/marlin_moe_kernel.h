...
template <vllm_dtype::ScalarTypeId w_type_id>
...
template <>
__device__ inline FragB dequant<vllm_dtype::kU4B8.id()>(int q) {
...
template <>
__device__ inline FragB dequant<vllm_dtype::kU8B128.id()>(int q) {
...
template <>
__device__ inline FragB dequant<vllm_dtype::kU4.id()>(int q) {
  const int LO = 0x000f000f;
  const int HI = 0x00f000f0;
  const int EX = 0x64006400;
  // Guarantee that the `(a & b) | c` operations are LOP3s.
...
template <>
__device__ inline FragB dequant<vllm_dtype::kU8.id()>(int q) {
  static constexpr uint32_t mask_for_elt_01 = 0x5250;
  static constexpr uint32_t mask_for_elt_23 = 0x5351;
  static constexpr uint32_t start_byte_for_fp16 = 0x64646464;
...
__device__ inline void scale4(FragB& frag_b, FragS& frag_s_1, FragS& frag_s_2, FragS& frag_s_3, FragS& frag_s_4,
                              int i) {
  __half2 s_val_1_2;
...
template <const vllm_dtype::ScalarTypeId w_type_id,  // weight ScalarType id
          ...
          >
__device__ void MarlinMoESingle(const int4* __restrict__ A,              // fp16 input matrix of shape mxk
  ...
  static constexpr auto w_type = vllm_dtype::ScalarType::from_id(w_type_id);
...
#  pragma unroll
  ...
  auto zero_accums = [&]() {
#  pragma unroll
    for (int i = 0; i < thread_m_blocks * 4 * 2 * 4; i++) reinterpret_cast<float*>(frag_c)[i] = 0;
  };
...
#  pragma unroll
      for (int i = 0; i < a_sh_wr_iters; i++) {
        ...
        if (sorted_row < tot_m * (replicate_input ? 1 : topk) &&
          ...
          cp_async4_pred(&sh_a_stage[a_sh_wr_trans[i]], &A[new_idx], a_sh_wr_pred[i]);
        }
      }
...
#  pragma unroll
  ...
  auto fetch_to_registers = [&](int k, int pipe) {
    int4* sh_a_stage = sh_a + a_sh_stage * pipe;
#  pragma unroll
    for (int i = 0; i < thread_m_blocks; i++) ldsm4(frag_a[k % 2][i], &sh_a_stage[a_sh_rd_trans[k % b_sh_wr_iters][i]]);
    int4* sh_b_stage = sh_b + b_sh_stage * pipe;
...
#  pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      if constexpr (has_act_order) {
        scale4(frag_b0, act_frag_s[k % 2][0][j], act_frag_s[k % 2][1][j], act_frag_s[k % 2][2][j],
      ...
      if constexpr (has_act_order) {
        scale4(frag_b1, act_frag_s[k % 2][0][j], act_frag_s[k % 2][1][j], act_frag_s[k % 2][2][j],
...
#  pragma unroll
            for (int j = 0; j < 4; j++) reinterpret_cast<FragC*>(frag_c)[4 * 2 * m_block + i][j] += c_rd[j];
          }
        }
        __syncthreads();
      }
        ...
        for (int i = 0; i < thread_m_blocks * 4; i++) {
          ...
          cp_async4_pred(&sh[c_sh_wr + c_sh_wr_delta * i], &C[new_idx],
                         sorted_row < tot_m * topk &&
                             (8 * (i / 2) + row < prob_m &&
                              (i < (thread_m_blocks - 1) * 4 || sorted_ids[8 * (i / 2) + row] < tot_m * topk)));
        }
        cp_async_fence();
        cp_async_wait<0>();
      }
...
#  pragma unroll
            for (int j = 0; j < 2 * 4; j++) {
              reinterpret_cast<__half*>(&c)[j] =
                  __float2half(reinterpret_cast<float*>(&frag_c)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4)]);
            }
            int c_idx = c_gl_wr + c_gl_wr_delta_o * (i / 2) + c_gl_wr_delta_i * (i % 2);
  ...
  auto write_result = [&]() {
    ...
    auto write = [&](int idx, float c0, float c1, FragS& s) {
      ...
      // 4-bit)
      if constexpr (!has_act_order && group_blocks == -1 && w_type.size_bits() == 4) {
        res = __hmul2(res, s[0]);
      }
...
#  pragma unroll
    ...
    if (slice_iters == 0) {
      ...
      // overflow in fp16)
      if constexpr (!has_act_order && group_blocks == -1 && w_type.size_bits() == 8) {
        if (threadIdx.x / 32 < thread_n_blocks / 4) {
#  pragma unroll
          for (int i = 0; i < thread_m_blocks; i++) {
#  pragma unroll
            for (int j = 0; j < 4; j++) {
              scale_float(reinterpret_cast<float*>(&frag_c[i][j][0][0]), frag_s[j / 2][2 * (j % 2) + 0]);
              scale_float(reinterpret_cast<float*>(&frag_c[i][j][0][2]), frag_s[j / 2][2 * (j % 2) + 0]);

              scale_float(reinterpret_cast<float*>(&frag_c[i][j][1][0]), frag_s[j / 2][2 * (j % 2) + 1]);
              scale_float(reinterpret_cast<float*>(&frag_c[i][j][1][2]), frag_s[j / 2][2 * (j % 2) + 1]);
            }
          }
        }
      }
...
template <const vllm_dtype::ScalarTypeId w_type_id,  // weight ScalarType id
          ...
          const int group_blocks = -1                // number of consecutive 16x16 blocks
                                                     // with a separate quantization scale
          >
__global__ void MarlinMoE(const int4* __restrict__ A,               // fp16 input matrix of shape mxk
                          const int4* __restrict__ B,               // 4bit quantized weight matrix of shape kxn
                          int4* __restrict__ C,                     // fp16 output buffer of shape mxn
  ...
  const int* sorted_ids_expert = sorted_ids_base + expert_offsets[expert_idx] + m_block_ctr * 4 * max_par;
  ...
  if (max_block == 1) {
    MarlinMoESingle<w_type_id, threads, 1, thread_n_blocks, thread_k_blocks, stages, has_act_order, has_zp,
                    group_blocks>(A, B, C, sorted_ids_expert, topk_weights, scales_ptr, zp_ptr, g_idx, expert_offsets,
                                  num_groups, expert_idx, num_experts, topk, prob_m, prob_n, prob_k, tot_m, locks,
                                  replicate_input, apply_weights, current_m_block);
  } else if (max_block == 2) {
    MarlinMoESingle<w_type_id, threads, 2, thread_n_blocks, thread_k_blocks, stages, has_act_order, has_zp,
                    group_blocks>(A, B, C, sorted_ids_expert, topk_weights, scales_ptr, zp_ptr, g_idx, expert_offsets,
                                  num_groups, expert_idx, num_experts, topk, prob_m, prob_n, prob_k, tot_m, locks,
                                  replicate_input, apply_weights, current_m_block);
  } else if (max_block == 3) {
    MarlinMoESingle<w_type_id, threads, 3, thread_n_blocks, thread_k_blocks, stages, has_act_order, has_zp,
                    group_blocks>(A, B, C, sorted_ids_expert, topk_weights, scales_ptr, zp_ptr, g_idx, expert_offsets,
                                  num_groups, expert_idx, num_experts, topk, prob_m, prob_n, prob_k, tot_m, locks,
                                  replicate_input, apply_weights, current_m_block);
  } else {
    MarlinMoESingle<w_type_id, threads, 4, thread_n_blocks, thread_k_blocks, stages, has_act_order, has_zp,
                    group_blocks>(A, B, C, sorted_ids_expert, topk_weights, scales_ptr, zp_ptr, g_idx, expert_offsets,
                                  num_groups, expert_idx, num_experts, topk, prob_m, prob_n, prob_k, tot_m, locks,
                                  replicate_input, apply_weights, current_m_block);
  }
...
#define __CALL_IF_MOE(W_TYPE, THREAD_N_BLOCKS, THREAD_K_BLOCKS, HAS_ACT_ORDER, HAS_ZP, GROUP_BLOCKS, NUM_THREADS)      \
  else if (q_type == W_TYPE && thread_n_blocks == THREAD_N_BLOCKS && thread_k_blocks == THREAD_K_BLOCKS &&             \
           has_act_order == HAS_ACT_ORDER && has_zp == HAS_ZP && group_blocks == GROUP_BLOCKS &&                       \
           num_threads == NUM_THREADS) {                                                                               \
    cudaFuncSetAttribute(MarlinMoE<W_TYPE.id(), NUM_THREADS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, HAS_ACT_ORDER,  \
                                   HAS_ZP, GROUP_BLOCKS>,                                                              \
                         cudaFuncAttributeMaxDynamicSharedMemorySize, max_shared_mem);                                 \
    MarlinMoE<W_TYPE.id(), NUM_THREADS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, HAS_ACT_ORDER, HAS_ZP, GROUP_BLOCKS> \
        <<<blocks, NUM_THREADS, max_shared_mem, stream>>>(                                                             \
            A_ptr, B_ptr, C_ptr, sorted_ids_ptr, topk_weights_ptr, s_ptr, zp_ptr, g_idx_ptr, expert_offsets_ptr,       \
            num_groups, expert_idx, num_experts, topk, prob_m, prob_n, prob_k, tot_m, locks, replicate_input,          \
            apply_weights, m_block, max_par, cfg_max_m_blocks);                                                        \
  }
