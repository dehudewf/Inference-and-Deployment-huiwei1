...
TllmException::TllmException(char const* file, std::size_t line, char const* msg) : std::runtime_error{""} {
...
std::string TllmException::getTrace() const {
  ...
  auto const trace = std::unique_ptr<char const*, void (*)(char const**)>(
      const_cast<char const**>(backtrace_symbols(mCallstack.data(), mNbFrames)), [](char const** p) { std::free(p); });
  if (trace == nullptr) {
    throw std::bad_alloc();
  }
