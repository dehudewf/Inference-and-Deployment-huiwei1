...
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_extensions/gemm/collective/collective_builder_mixed_input.hpp"
...
void sm90_generic_mixed_moe_gemm_kernelLauncher(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs,
  ...
  using CollectiveMainloop = typename cutlass::gemm::collective::CollectiveBuilderMixedInput<
      ArchTag, OperatorClass, cute::tuple<ElementB, ElementScalePacked>, LayoutB_Transpose*, AlignmentB, ElementA,
  ...
  arguments = Args{
      ...
      {reinterpret_cast<ElementB const**>(hopper_inputs.ptr_weight),
       ...
       reinterpret_cast<ElementA const**>(hopper_inputs.ptr_act), reinterpret_cast<StrideA*>(hopper_inputs.stride_act),
  ...
  KLLM_KERNEL_CHECK_WITH_INFO(hopper_inputs.swap_ab, "swap_ab must be true for mixed dtype WS grouped GEMM");
