...
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_extensions/gemm/device/gemm_universal_base_compat.h"
...
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_extensions/gemm/kernel/fpA_intB_gemm.h"
...
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_kernels/fpA_intB_gemm/fpA_intB_gemm_template_sm90.h"
...
#  endif
  ...
  using EpilogueOp = typename tkc::Epilogue<CutlassOutputType, ElementsPerAccessC, ElementAccumulator, EpilogueTag>::Op;
  ...
  using GemmKernel_ = typename cutlass::gemm::kernel::DefaultGemm<
      ...
      typename cutlass::gemm::threadblock::GemmIdentityThreadblockSwizzle<8>, Stages, true, TaggedOperator>::GemmKernel;
  ...
  if (occupancy != nullptr) {
    *occupancy = llm_kernels::nvidia::tensorrt_llm::dev::cutlass_extensions::compute_occupancy_for_kernel<GemmKernel>();
    return;
  }
  ...
  if (gemm.get_workspace_size(args) > workspace_bytes) {
    TLLM_LOG_WARNING("Requested split-k but workspace size insufficient. Falling back to non-split-k implementation.");
...
size_t CutlassFpAIntBGemmRunner<ActivationType, WeightType, QuantOp, ScaleZeroType, BiasType,
  ...
  if (sm_ == 90) {
    ...
    return static_cast<size_t>(max_sk_tiles_with_seperate_reduction * MAX_M_TILE_SM90 * MAX_N_TILE_SM90 *
                               sizeof(float));
  }
