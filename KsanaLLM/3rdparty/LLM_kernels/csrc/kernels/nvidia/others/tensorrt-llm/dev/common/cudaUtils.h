...
void check(T ptr, char const* const func, char const* const file, int const line) {
  if (ptr) {
    throw TllmException(
        file, line, fmtstr("[TensorRT-LLM][ERROR] CUDA runtime error in %s: %s", func, _cudaGetErrorEnum(ptr)).c_str());
  }
}

template <typename T>
void checkEx(T ptr, std::initializer_list<T> const& validReturns, char const* const func, char const* const file,
             int const line) {
...
#define sync_stream_check_cuda_error(stream) \
  llm_kernels::nvidia::tensorrt_llm::dev::common::syncAndCheck(stream, __FILE__, __LINE__)
...
__host__ __device__ inline void print_element_(bool ui) { printf("%7" PRIu32 " ", (unsigned int)ui); }

__host__ __device__ inline void print_element_(uint8_t ui) { printf("%7" PRIu32 " ", (unsigned int)ui); }
...
struct Arrive_wait {
 public:
  ...
  __forceinline__ __device__ void set_bar_base_dsmem(uint32_t const& cta_id) {
    bar_base_ =
        reinterpret_cast<uint64_t*>((reinterpret_cast<uintptr_t>(bar_base_) & 0xFFFFFFFFF0FFFFFFULL) + (cta_id << 24));
  }
...
__forceinline__ __device__ void arrive_DSMEM_barrier_and_set_tx_cnt(uint64_t* p_barrier, uint32_t ctaid,
...
#define TLLM_CUDA_CHECK(stat)                                                                 \
  do {                                                                                        \
    llm_kernels::nvidia::tensorrt_llm::dev::common::check((stat), #stat, __FILE__, __LINE__); \
  } while (0)
...
#define TLLM_CUDA_CHECK_FREE_RESOURCE(stat)                                                                         \
  do {                                                                                                              \
    llm_kernels::nvidia::tensorrt_llm::dev::common::checkEx((stat), {cudaSuccess, cudaErrorCudartUnloading}, #stat, \
                                                            __FILE__, __LINE__);                                    \
  } while (0)
