...
std::array<size_t, 20> TmaWarpSpecializedGroupedGemmInput::workspaceBuffers(int num_experts,
...
size_t TmaWarpSpecializedGroupedGemmInput::workspaceSize(int num_experts, FpXBlockScalingType scaling_type) {
  ...
  return llm_kernels::nvidia::tensorrt_llm::dev::common::calculateTotalWorkspaceSize(buffers.data(), buffers.size());
}
...
void TmaWarpSpecializedGroupedGemmInput::configureWorkspace(int8_t* start_ptr, int num_experts, void* gemm_workspace,
  ...
  KLLM_KERNEL_CHECK_WITH_INFO(pointers.size() == buffers.size(), "Mismatching workspace size and number of buffers");
  for (int i = 0; i < buffers.size(); i++) {
    pointers[i] = start_ptr;
    start_ptr = llm_kernels::nvidia::tensorrt_llm::dev::common::nextWorkspacePtr(start_ptr, buffers[i]);
  }
...
void TmaWarpSpecializedGroupedGemmInput::setFinalizeFusionParams(void* final_output, int hidden_size,
                                                                 int num_output_tokens, bool use_reduction) {
