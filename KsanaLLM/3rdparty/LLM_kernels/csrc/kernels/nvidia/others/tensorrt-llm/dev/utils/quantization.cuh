...
#ifdef ENABLE_BF16
__global__ static void quantizedKernel(char4* dst, __nv_bfloat162 const* src, int64_t const sizeDiv4,
...
inline __device__ void quantizeAndStore(QuantT* dstPtr, uint4 vec, T const clampMin, T const clampMax,
...
inline __host__ __device__ int64_t get_sf_out_offset_128x4(std::optional<int> batchIdx, int mIdx, int kIdx,
...
__device__ uint8_t* cvt_quant_get_sf_out_offset(std::optional<int> batchIdx, int rowIdx, int colVecIdx,
                                                std::optional<int> numRows, int numColVecs, SFType* SFout,
...
#endif
    int32_t numbatches, int32_t numRows, int32_t numCols, int32_t numPaddedCols, Type const* in, float const* SFScale,
      ...
      for (int colIdx = threadIdx.x; colIdx < numColThreadsForSf; colIdx += blockDim.x) {
        ...
        if (rowIdx >= numRows || colIdx >= numColThreads) {
          ...
          if constexpr (quantization_type == BlockScaleQuantizationType::FP16_TO_FP4) {
            ...
            reinterpret_cast<uint64_t*>(out)[outOffset] = cvt_warp_fp16_to_mxfp8<Type, SF_VEC_SIZE>(in_vec, sf_out);
          }
        }
      }
    }
...
__global__ void block_scale_interleave_kernel(int numbatches, int numRows, int numCols, uint8_t const* SFIn,
                                              uint8_t* SFOutput);
}  // namespace kernels
}  // namespace llm_kernels::nvidia::tensorrt_llm::dev
