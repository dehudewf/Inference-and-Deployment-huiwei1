...
#pragma once
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_extensions/gemm/kernel/fused_moe_kernel_traits.cuh"
...
struct Fused_Moe_Kernel_routine_sm80<ElementInput_, ElementWeight_, ElementOutput_, TileM_, TileN_, TileK_, Stages_,
  ...
  CUTE_DEVICE auto gmem_tensor_init(int const problem_index, int const gemm_m, Params const& params) {
    ...
    cute::Tensor gInput_mk =
        cute::local_tile(mInput_mk, typename KT::TileShape{}, cute::make_coord(cute::_, cute::_, cute::_),
                         cute::Step<cute::_1, X, cute::_1>{});  // (BLK_M, BLK_K, m, k)
    cute::Tensor gfc1_gate_nk =
        cute::local_tile(mfc1_gate_nk, typename KT::TileShape{}, cute::make_coord(cute::_, cute::_, cute::_),
                         cute::Step<X, cute::_1, cute::_1>{});  // (BLK_N, BLK_K, n, k)
    cute::Tensor gfc1_nk =
        cute::local_tile(mfc1_nk, typename KT::TileShape{}, cute::make_coord(cute::_, cute::_, cute::_),
                         cute::Step<X, cute::_1, cute::_1>{});  // (BLK_N, BLK_K, n, k)

    cute::Tensor gBias_mn =
        cute::local_tile(mBias_mn, typename KT::TileShape{}, cute::make_coord(cute::_, cute::_, cute::_),
                         cute::Step<cute::_1, cute::_1, X>{});  // (BLK_M, BLK_N, m, n)

    cute::Tensor gBias_gate_mn =
        cute::local_tile(mBias_gate_mn, typename KT::TileShape{}, cute::make_coord(cute::_, cute::_, cute::_),
                         cute::Step<cute::_1, cute::_1, X>{});  // (BLK_M, BLK_N, m, n)

    cute::Tensor gOutput_mn =
        cute::local_tile(mOutput_mn, typename KT::TileShape{}, cute::make_coord(cute::_, cute::_, cute::_),
                         cute::Step<cute::_1, cute::_1, X>{});  // (BLK_M, BLK_N, m, n)
  ...
  CUTE_DEVICE void run_routine(Params const& params, int const problem_index, int const block_m_idx,
    ...
    cute::Tensor sfc1_gate_weight = cute::make_tensor(cute::make_smem_ptr(shared_storage.smem_fc1_gate_weight.data()),
                                                      typename KT::SmemLayoutB{});  // (BLK_N, BLK_K, Stage)
    ...
    cute::Tensor tInputpInput = cute::make_tensor<bool>(
        cute::make_shape(cute::size<1>(tInputsInput), cute::size<2>(tInputsInput)), cute::Stride<cute::_1, cute::_0>{});
    ...
    for (; k_tile_count > 0; --k_tile_count) {
      cute::for_each(cute::make_int_sequence<K_BLOCK_MAX>{}, [&](auto k_block) {
        ...
        cute::gemm(tiled_mma, accum, tOrInput(cute::_, cute::_, k_block), tOrfc1(cute::_, cute::_, k_block), accum);
        cute::gemm(tiled_mma, accum_gate, tOrInput(cute::_, cute::_, k_block), tOrfc1g(cute::_, cute::_, k_block),
                   accum_gate);
      });
    }
