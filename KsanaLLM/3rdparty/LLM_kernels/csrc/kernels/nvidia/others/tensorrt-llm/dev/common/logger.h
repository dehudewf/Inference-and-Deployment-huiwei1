...
#  undef ERROR
 ...
 private:
  ...
  static inline char const* getLevelName(Level const level) {
    ...
    KLLM_KERNEL_THROW("Unknown log level: %d", level);
  }

  static inline std::string getPrefix(Level const level) { return fmtstr("%s[%s] ", kPREFIX, getLevelName(level)); }
...
#define TLLM_LOG(level, ...)                                                                  \
  do {                                                                                        \
    auto* const logger = llm_kernels::nvidia::tensorrt_llm::dev::common::Logger::getLogger(); \
    if (logger->isEnabled(level)) {                                                           \
      logger->log(level, __VA_ARGS__);                                                        \
    }                                                                                         \
  } while (0)

#define TLLM_LOG_TRACE(...) TLLM_LOG(llm_kernels::nvidia::tensorrt_llm::dev::common::Logger::TRACE, __VA_ARGS__)
#define TLLM_LOG_DEBUG(...) TLLM_LOG(llm_kernels::nvidia::tensorrt_llm::dev::common::Logger::DEBUG, __VA_ARGS__)
#define TLLM_LOG_INFO(...) TLLM_LOG(llm_kernels::nvidia::tensorrt_llm::dev::common::Logger::INFO, __VA_ARGS__)
#define TLLM_LOG_WARNING(...) TLLM_LOG(llm_kernels::nvidia::tensorrt_llm::dev::common::Logger::WARNING, __VA_ARGS__)
#define TLLM_LOG_ERROR(...) TLLM_LOG(llm_kernels::nvidia::tensorrt_llm::dev::common::Logger::ERROR, __VA_ARGS__)
#define TLLM_LOG_EXCEPTION(ex, ...) \
  llm_kernels::nvidia::tensorrt_llm::dev::common::Logger::getLogger()->log(ex, ##__VA_ARGS__)
}  // namespace llm_kernels::nvidia::tensorrt_llm::dev::common
