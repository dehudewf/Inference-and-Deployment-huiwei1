...
#include <random>
#include <sstream>
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/common/memoryUtils.h"
...
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/common/dataType.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/common/envUtils.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_kernels/cutlass_type_conversion.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_kernels/include/moe_kernels.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/utils/preQuantScaleKernel.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/utils/quantization.cuh"

#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_kernels/include/moe_util_kernels.h"
...
using namespace llm_kernels::nvidia::tensorrt_llm::dev::kernels;
using namespace llm_kernels::nvidia::tensorrt_llm::dev::common;
...
__device__ __forceinline__ void setLocalExperts(int* s_local_experts, T const* token_selected_experts,
                                                int const total_num_experts, int const tid, int const start_expert,
...
__global__ void buildMinLatencyActiveExpertMapsKernel(int* num_active_experts_per_node, float* experts_to_token_scores,
                                                      ...
                                                      int const num_experts_per_token, int const start_expert,
                                                      int const end_expert, int const num_experts_per_node,
                                                      bool const smart_routing, int const cluster_rank,
  ...
  setLocalExperts<int, BLOCK_SIZE>(s_local_experts, token_selected_experts, total_num_experts, tid, start_expert,
                                   end_expert);
...
void buildMinLatencyActiveExpertMaps(int* num_active_experts_per_node, float* experts_to_token_scores,
                                     ...
                                     int const end_expert, int const num_experts_per_node, int const cluster_rank,
                                     int const cluster_size, int const num_experts_smem, cudaStream_t const stream) {
  KLLM_KERNEL_CHECK_WITH_INFO(num_experts_per_node == (end_expert - start_expert),
                              "num_experts_per_node must be equal to end_expert - start_expert");

  KLLM_KERNEL_CHECK_WITH_INFO(num_experts_per_node <= 256, "don't support num_experts_per_node > 256 cases");
  ...
  attrs[0].val.programmaticStreamSerializationAllowed =
      llm_kernels::nvidia::tensorrt_llm::dev::common::getEnvEnablePDL();
  config.numAttrs = 1;
  config.attrs = attrs;
  cudaLaunchKernelEx(&config, buildMinLatencyActiveExpertMapsKernel<threads>, num_active_experts_per_node,
                     experts_to_token_scores, active_expert_global_ids, expert_first_token_offset,
                     token_selected_experts, token_final_scales, num_tokens, experts_per_token, start_expert,
                     end_expert, num_experts_per_node, smart_routing, cluster_rank, cluster_size, num_experts_smem);
}
...
__global__ void fusedBuildExpertMapsSortFirstTokenKernel(
    ...
    int* const unpermuted_row_to_permuted_row, int64_t* const expert_first_token_offset, int64_t const num_tokens,
    int const experts_per_token, int const start_expert, int const end_expert, int const num_experts_per_node) {
...
#pragma unroll
  ...
  BlockRadixRank(sort_temp).RankKeys(local_token_selected_experts, local_token_permuted_indices, extractor,
                                     local_expert_first_token_offset);
...
bool fusedBuildExpertMapsSortFirstTokenDispatch(int const* token_selected_experts, int* permuted_row_to_unpermuted_row,
  ...
  KLLM_KERNEL_CHECK_WITH_INFO(blocks == 1, "Current implementation requires single block");
...
bool fusedBuildExpertMapsSortFirstTokenBlockSize(int const* token_selected_experts, int* permuted_row_to_unpermuted_row,
  ...
  if (num_tokens > 256) {
    TLLM_LOG_TRACE("Number of tokens %d is greater than 256, which is not supported for fused moe prologues",
                   num_tokens);
    return false;
  }
...
bool fusedBuildExpertMapsSortFirstToken(int const* token_selected_experts, int* permuted_row_to_unpermuted_row,
                                        int* unpermuted_row_to_permuted_row, int64_t* expert_first_token_offset,
                                        int64_t const num_tokens, int const num_experts_per_node,
                                        int const experts_per_token, int const start_expert, int const end_expert,
                                        cudaStream_t stream) {
  ...
  if (expert_log <= 9) {
    auto funcs =
        std::array{&fusedBuildExpertMapsSortFirstTokenBlockSize<1>, &fusedBuildExpertMapsSortFirstTokenBlockSize<2>,
                   &fusedBuildExpertMapsSortFirstTokenBlockSize<3>, &fusedBuildExpertMapsSortFirstTokenBlockSize<4>,
                   &fusedBuildExpertMapsSortFirstTokenBlockSize<5>, &fusedBuildExpertMapsSortFirstTokenBlockSize<6>,
                   &fusedBuildExpertMapsSortFirstTokenBlockSize<7>, &fusedBuildExpertMapsSortFirstTokenBlockSize<8>,
                   &fusedBuildExpertMapsSortFirstTokenBlockSize<9>};

    return funcs[expert_log - 1](token_selected_experts, permuted_row_to_unpermuted_row, unpermuted_row_to_permuted_row,
...
int64_t computeNumTokensPerBlock(int64_t const num_tokens, int64_t const num_experts_per_node) {
  for (int64_t num_tokens_per_block = 32; num_tokens_per_block <= 1024; num_tokens_per_block *= 2) {
    int64_t const num_blocks_per_seq =
        llm_kernels::nvidia::tensorrt_llm::dev::common::ceilDiv(num_tokens, num_tokens_per_block);
...
__global__ void globalExpertPrefixSumLargeKernel(int const* blocked_expert_counts, int* blocked_expert_counts_cumsum,
                                                 ...
                                                 int64_t const num_blocks_per_seq, int64_t const num_elem_per_thread) {
...
void globalExpertPrefixSum(int const* blocked_expert_counts, int* blocked_expert_counts_cumsum,
  ...
  if (num_elements <= 1024) {
    ...
    int64_t const num_elem_per_thread = llm_kernels::nvidia::tensorrt_llm::dev::common::ceilDiv(num_elements, 1024);
    cudaLaunchKernelEx(&config, func, blocked_expert_counts, blocked_expert_counts_cumsum, expert_first_token_offset,
                       num_experts_per_node, num_blocks_per_seq, num_elem_per_thread);
  }
}
...
void threeStepBuildExpertMapsSortFirstToken(int const* token_selected_experts, int* permuted_token_selected_experts,
  ...
  sync_stream_check_cuda_error(stream);
...
__device__ auto quantizePackedFPXValue(ComputeElem& post_act_val, float global_scale_val,
                                       int64_t num_tokens_before_expert, int64_t expert_id, int64_t token_id,
                                       int64_t elem_idx, int64_t num_cols,
  ...
  if (sf_out) {
    if (input_sf) {
      if (swizzled_input_sf) {
        auto const sf_in = cvt_quant_get_sf_out_offset<TmaWarpSpecializedGroupedGemmInput::ElementSF, NumThreadsPerSF>(
            std::nullopt /* batchIdx */, source_token_id, elem_idx, std::nullopt /* numRows */, num_cols / VecSize,
            const_cast<TmaWarpSpecializedGroupedGemmInput::ElementSF*>(input_sf), QuantizationSFLayout::SWIZZLED);
        *sf_out = *sf_in;
      } else {
        auto const sf_in = cvt_quant_get_sf_out_offset<TmaWarpSpecializedGroupedGemmInput::ElementSF, NumThreadsPerSF>(
            std::nullopt /* batchIdx */, source_token_id, elem_idx, std::nullopt /* numRows */, num_cols / VecSize,
            const_cast<TmaWarpSpecializedGroupedGemmInput::ElementSF*>(input_sf), QuantizationSFLayout::LINEAR);
        *sf_out = *sf_in;
      }
    } else {
      *sf_out = 0x00;
...
__global__ void computeFP8DequantScaleKernel(float const** alpha_scale_ptr_array, int64_t const num_experts_per_node,
...
float const** computeFP8DequantScale(float const** alpha_scale_ptr_array, int const num_experts_per_node,
  ...
  computeFP8DequantScaleKernel<<<blocks, threads, 0, stream>>>(alpha_scale_ptr_array, num_experts_per_node,
                                                               fp8_dequant);
...
__device__ void computeTmaWarpSpecializedInputStrides(TmaWarpSpecializedGroupedGemmInput& layout_info, int gemm_m,
                                                      int gemm_n, int gemm_k, int64_t out_idx) {
  if (layout_info.swap_ab) {
  ...
  if (layout_info.int4_groupwise_params.enabled) {
    layout_info.int4_groupwise_params.stride_s_a[out_idx] = cutlass::make_cute_packed_stride(
        ...
        cute::make_shape(gemm_n,
                         gemm_k / (layout_info.int4_groupwise_params.use_wfp4a16
...
__device__ void computeTmaWarpSpecializedInputPointers(
    ...
    TmaWarpSpecializedGroupedGemmInput::INT4GroupwiseParams::SFA const* w4a8_weight_scale, ScaleBiasType const* bias,
    OutputType* output, float const* router_scales, int const* permuted_row_to_unpermuted_row, int64_t const out_idx) {
  ...
  if (layout_info.int4_groupwise_params.enabled) {
    ...
    layout_info.int4_groupwise_params.ptr_s_a[out_idx] =
        safe_inc_ptr(w4a8_weight_scale,
                     expert * (gemm_n * gemm_k /
...
__global__ void computeStridesTmaWarpSpecializedKernel(
    ...
    int64_t gemm1_k, int64_t gemm2_n, int64_t gemm2_k, int64_t const num_experts_per_node, T const* gemm1_in,
    T const* gemm2_in, WeightType const* weights1, WeightType const* weights2, float const* alpha_scale_flat1,
  ...
  auto setupIfSelected = [&](auto bs_config, auto quant_type) {
    if (quant_type.fc1.weight_block_scale) {
      setupFP4BlockScalingFactors<decltype(bs_config)>(layout_info1, expert, gemm_m, gemm1_n, gemm1_k, fp4_act_flat1,
                                                       quant_type.fc1.weight_block_scale, num_tokens_before_expert);
    }
    if (quant_type.fc2.weight_block_scale) {
      setupFP4BlockScalingFactors<decltype(bs_config)>(layout_info2, expert, gemm_m, gemm2_n, gemm2_k, fp4_act_flat2,
...
__global__ void expandInputRowsKernel(InputActivationsType const* unpermuted_input,
                                      ...
                                      TmaWarpSpecializedGroupedGemmInput::ElementSF const* input_sf,
                                      bool const swizzled_input_sf, int64_t const num_experts_per_node,
  ...
  // NOTE(jinxcwu) 遵循act_scale和alpha都只有一个有效值的假设进行优化
  register const InputActivationsType prequant_scale = prequant_scales[0];
  ...
  for (int64_t permuted_row = blockIdx.x; permuted_row < num_valid_tokens; permuted_row += gridDim.x) {
    ...
    if constexpr (is_nvfp4 || is_mxfp8) {
      ...
      int64_t expert =
          findTotalEltsLessThanTarget(expert_first_token_offset, num_experts_per_node, (int64_t)permuted_row + 1) - 1;
      ...
      for (int elem_index = start_offset; elem_index < num_elems_in_col; elem_index += stride) {
        ...
        if constexpr (need_nvfp4_quant || need_mxfp8_quant) {
          auto res = quantizePackedFPXValue<InputActivationsType, ExpandedActivationsType, DataElem, VecSize>(
              in_vec, global_scale_val, num_tokens_before_expert, expert, permuted_row, elem_index, padded_hidden_size,
              fc1_act_sf_flat,
              is_nvfp4 ? TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::NVFP4
                       : TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::MXFPX);
          static_assert(sizeof(res) == sizeof(*dest_row_ptr), "Quantized value must be the same size as the output");
          dest_row_ptr[elem_index] = res;
        } else {
          assert(act_scale_idx == 0 && "Cannot use per-expert act scale for pre-quantized activations");
          writeSF<VecSize, ELEM_PER_THREAD>(num_tokens_before_expert, expert, source_row, permuted_row, elem_index,
                                            padded_hidden_size, fc1_act_sf_flat, input_sf, swizzled_input_sf);
      ...
      for (int elem_index = start_offset; elem_index < num_elems_in_col; elem_index += stride) {
        ...
        frag_elems = frag_elems * prequant_scale;
      ...
      if (expert_pad_idx < padding_to_expert) {
        for (int64_t elem_index = start_offset; elem_index < padded_num_elems_in_col; elem_index += stride) {
          writeSF<VecSize, VecSize>(num_tokens_before_expert, expert, /*source_row*/ -1,
                                    num_tokens_after_expert + expert_pad_idx, elem_index, padded_hidden_size,
                                    fc1_act_sf_flat,
                                    /* input_sf */ nullptr);  // Pass nulltpr input_sf so we write 0
        }
      }
...
void expandInputRowsKernelLauncher(InputActivationsType const* unpermuted_input,
  ...
  static int64_t const smCount = llm_kernels::nvidia::tensorrt_llm::dev::common::getMultiProcessorCount();
  ...
  auto func = [&]() {
    ...
    if constexpr (std::is_same_v<ExpandedActivationsType, __nv_fp8_e4m3> &&
      ...
      KLLM_KERNEL_CHECK_WITH_INFO(
          quant_params.mxfp8_mxfp4.fc1.weight_block_scale || prequant_scales,
    ...
    // Could be either regular FP8 or MXFP8
                       ...
                       std::is_same_v<InputActivationsType, __nv_fp8_e4m3>) {
      KLLM_KERNEL_CHECK_WITH_INFO(!prequant_scales, "NVFP4 is not supported for AWQ");
      ...
      KLLM_KERNEL_CHECK_WITH_INFO(quant_params.fp4.fc1.weight_block_scale,
...
#endif
    {
      KLLM_KERNEL_CHECK_WITH_INFO(!prequant_scales, "w4afp8 Prequant scales provided for non-FP8 data type");
      return &expandInputRowsKernel<InputActivationsType, ExpandedActivationsType,
                                    TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::NONE, false>;
    }
  }();
...
__global__ void finalizeMoeRoutingKernel(GemmOutputType const* expanded_permuted_rows,
                                         ...
                                         float const* scales, int const* unpermuted_row_to_permuted_row,
                                         int const* token_selected_experts, int64_t const padded_cols,
...
__global__ void finalizeMoeRoutingNoFillingKernel(
    GemmOutputType const* expanded_permuted_rows, OutputType* reduced_unpermuted_output, ScaleBiasType const* bias,
    float const* scales, int const* const unpermuted_row_to_permuted_row, int const* permuted_row_to_unpermuted_row,
...
void finalizeMoeRoutingKernelLauncher(GemmOutputType const* expanded_permuted_rows,
  ...
  if (parallelism_config.ep_size > 1 && enable_alltoall) {
    ...
    static int const smCount = llm_kernels::nvidia::tensorrt_llm::dev::common::getMultiProcessorCount();
    // Note: Launching 8 blocks per SM can fully leverage the memory bandwidth (tested on B200).
    ...
    cudaLaunchKernelEx(&config, func, expanded_permuted_rows, reduced_unpermuted_output, bias_ptr, final_scales,
                       ...
                       expert_first_token_offset, num_rows, padded_cols, unpadded_cols, experts_per_token,
    ...
    cudaLaunchKernelEx(&config, func, expanded_permuted_rows, reduced_unpermuted_output, bias_ptr, final_scales,
                       unpermuted_row_to_permuted_row, token_selected_experts, padded_cols, unpadded_cols,
...
#define INSTANTIATE_FINALIZE_MOE_ROUTING(OutputT, GemmOutputT, ScaleBiasT)                                             \
  template void finalizeMoeRoutingKernelLauncher<OutputT, GemmOutputT, ScaleBiasT>(                                    \
      ...
      float const* final_scales, int const* unpermuted_row_to_permuted_row, int const* permuted_row_to_unpermuted_row, \
      int const* expert_for_source_row, int64_t const* expert_first_token_offset, int64_t const num_rows,              \
      int64_t const padded_cols, int64_t const actual_cols, int64_t const experts_per_token,                           \
...
__global__ void doGatedActivationKernel(ActivationOutputType* output, GemmOutputType const* gemm_result,
                                        ...
                                        int64_t num_experts_per_node, ActivationParams activation_type) {
  ...
  if (activation_type.swiglu_alpha || activation_type.swiglu_beta || activation_type.swiglu_limit) {
    int expert = findTotalEltsLessThanTarget(expert_first_token_offset, num_experts_per_node, (int64_t)token + 1) - 1;
    ...
    gate_limit =
        activation_type.swiglu_limit ? activation_type.swiglu_limit[expert] : std::numeric_limits<float>::infinity();
  }
...
void doGatedActivation(ActivationOutputType* output, GemmOutputType const* gemm_result,
                       int64_t const* expert_first_token_offset, int64_t inter_size, int64_t num_tokens,
                       int64_t num_experts_per_node, ActivationParams activation_type, cudaStream_t stream) {
  ...
  auto* fn =
      ...
      : activation_type == ActivationType::SwigluBias
          ...
          : nullptr;
  KLLM_KERNEL_CHECK_WITH_INFO(fn != nullptr, "Invalid activation type");
...
__global__ void doActivationKernel(T* output, GemmOutputType const* gemm_result, float const* fp8_quant,
                                   ...
                                   int64_t inter_size, float const* fc2_act_global_scale, bool use_per_expert_act_scale,
                                   TmaWarpSpecializedGroupedGemmInput::ElementSF* fc2_act_sf_flat,
                                   ActivationParams activation_params,
                                   GemmOutputType const* prequant_scales = nullptr) {
#ifdef ENABLE_FP4
  ...
  constexpr int64_t ACTIVATION_ELEM_PER_THREAD =
      (IsNVFP4 || IsMXFP8) ? CVT_ELTS_PER_THREAD
...
#endif
  // NOTE(jinxcwu) 遵循act_scale和alpha都只有一个有效值的假设进行优化
  register const float prequant_scale = prequant_scales ? static_cast<float>(prequant_scales[0]) : 1.0f;
  for (int64_t token = blockIdx.x; token < num_valid_tokens; token += gridDim.x) {
    ...
    using GemmResultElem = cutlass::Array<GemmOutputType, ACTIVATION_ELEM_PER_THREAD>;
    using OutputElem =
        std::conditional_t<IsNVFP4, uint32_t,
    ...
    for (int64_t elem_index = start_offset; elem_index < num_elems_in_col; elem_index += stride) {
      ...
      auto post_act_val = gate_act * prequant_scale;
      ...
      if constexpr (IsNVFP4 || IsMXFP8) {
        ...
        auto res = quantizePackedFPXValue<GemmOutputType, T, ComputeElem, VecSize>(
            post_act_val, global_scale_val, num_tokens_before_expert, expert, token, elem_index, inter_size,
            fc2_act_sf_flat,
            IsNVFP4 ? TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::NVFP4
                    : TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::MXFPX);
        static_assert(sizeof(res) == sizeof(*output_vec), "Quantized value must be the same size as the output");
  ...
  if constexpr (IsNVFP4 || IsMXFP8) {
    ...
    constexpr int64_t min_num_tokens_alignment = IsNVFP4 ? TmaWarpSpecializedGroupedGemmInput::MinNDimAlignmentNVFP4
                                                         : TmaWarpSpecializedGroupedGemmInput::MinNDimAlignmentMXFPX;
    static_assert((min_num_tokens_alignment & (min_num_tokens_alignment - 1)) == 0,
                  "Min num tokens alignment must be a power of two");
    // Since we don't know a priori how much padding is needed we assume the max per expert
    ...
    for (int64_t padding_token = blockIdx.x; padding_token < num_padding_tokens; padding_token += gridDim.x) {
      ...
      if (expert_pad_idx < padding_to_expert) {
        for (int64_t elem_index = start_offset; elem_index < padded_num_elems_in_col; elem_index += stride) {
          ...
          writeSF<VecSize, VecSize>(num_tokens_before_expert, expert, /*source_row*/ -1,
                                    num_tokens_after_expert + expert_pad_idx, elem_index, padded_inter_size,
                                    fc2_act_sf_flat,
                                    /* input_sf */ nullptr);  // Pass nulltpr input_sf so we write 0
        }
      }
...
void doActivation(T* output, GemmOutputType const* gemm_result, float const* fp8_quant, ScaleBiasType const* bias,
                  bool bias_is_broadcast, int64_t const* expert_first_token_offset, int num_experts_per_node,
  ...
  auto fn = [&]() {
    auto fn = [&](auto block_scaling_type) {
      auto fn_list = std::array{
          ...
          &doActivationKernel<T, GemmOutputType, ScaleBiasType, IdentityAdaptor<cutlass::epilogue::thread::Identity>,
                              decltype(block_scaling_type)::value>  // Identity
      };
      return fn_list[static_cast<int>(activation_type.activation_type)];
    };
    auto NVFP4 = llm_kernels::nvidia::tensorrt_llm::dev::common::ConstExprWrapper<
        TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType,
        TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::NVFP4>{};
    auto MXFPX = llm_kernels::nvidia::tensorrt_llm::dev::common::ConstExprWrapper<
        TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType,
        TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::MXFPX>{};
    auto NONE = llm_kernels::nvidia::tensorrt_llm::dev::common::ConstExprWrapper<
        TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType,
        TmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::NONE>{};
#ifdef ENABLE_FP4
    if constexpr (std::is_same_v<T, __nv_fp4_e2m1>) {
      KLLM_KERNEL_CHECK_WITH_INFO(quant_params.fp4.fc2.weight_block_scale,
                                  "NVFP4 block scaling is expected for FP4xFP4");
      return fn(NVFP4);
  ...
  cudaLaunchKernelEx(&config, fn, output, gemm_result, fp8_quant, bias, bias_is_broadcast, expert_first_token_offset,
                     ...
                     reinterpret_cast<GemmOutputType const*>(quant_params.groupwise.fc2.act_scales));
}
...
__global__ void loraAddBiasKernel(ScaleBiasType* output, LoraType const* lora_result, ScaleBiasType const* bias,
                                  int64_t const* num_valid_tokens_ptr, int* permuted_token_selected_experts,
                                  int64_t inter_size) {
  int64_t const tid = threadIdx.x;
  int64_t const token = blockIdx.x;
...
void loraAddBias(ScaleBiasType* output, LoraType const* lora_result, ScaleBiasType const* bias,
                 int64_t const* num_valid_tokens_ptr, int64_t inter_size, int* permuted_token_selected_experts,
                 int64_t num_tokens, bool is_gated_activation, cudaStream_t stream) {
  int64_t const blocks = num_tokens;
...
CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::CutlassMoeFCRunner()
    : blockscale_gemm_runner_{
          std::make_unique<kernels::fp8_blockscale_gemm::CutlassFp8BlockScaleGemmRunner<__nv_bfloat16, __nv_fp8_e4m3,
                                                                                        __nv_bfloat16>>()} {}
...
CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::getWorkspaceDeviceBufferSizes(
  ...
  int64_t const num_blocks_per_seq =
      llm_kernels::nvidia::tensorrt_llm::dev::common::ceilDiv(num_rows, num_tokens_per_block);
  ...
  // tokens*topk
  auto act_sf_rows = min_latency_mode ? num_moe_inputs
  ...
  size_t const lora_fc1_result_size = use_lora ? (is_gated_activation ? 2 * interbuf_elems * sizeof(ScaleBiasType)
  ...
  if (use_deepseek_fp8_block_scale) {
    ...
    KLLM_KERNEL_CHECK(blockscale_gemm_runner != nullptr);
...
#define ADD_NAME(name, size)                                                       \
  do {                                                                             \
    auto aligned_size = llm_kernels::nvidia::tensorrt_llm::dev::common::alignSize( \
        size, llm_kernels::nvidia::tensorrt_llm::dev::common::kCudaMemAlign);      \
...
size_t CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::getWorkspaceSize(
  ...
  auto sizes_map =
      getWorkspaceDeviceBufferSizes(num_rows, hidden_size, inter_size, num_experts / ep_size, experts_per_token,
                                    activation_type, use_lora, use_deepseek_fp8_block_scale, min_latency_mode, use_awq);
  ...
  size_t size = llm_kernels::nvidia::tensorrt_llm::dev::common::calculateTotalWorkspaceSize(sizes.data(), sizes.size());
  TLLM_LOG_TRACE("Mixture Of Experts Plugin requires workspace of %2f MiB", size / 1024.f / 1024.f);
  return size;
}

template <class T, class WeightType, class OutputType, class InputType, class BackBoneType, class Enable>
void CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::configureWsPtrs(
    ...
    bool use_awq) {
  auto workspaces =
      getWorkspaceDeviceBufferSizes(num_rows, hidden_size, inter_size, num_experts_per_node, experts_per_token,
                                    activation_type, use_lora, use_deepseek_fp8_block_scale, min_latency_mode, use_awq);

  auto getWsPtr = [&](auto type, std::string const& name) {
    return workspaces.at(name).first ? reinterpret_cast<decltype(type)*>(ws_ptr + workspaces.at(name).second) : nullptr;
  };
  ...
  if (use_block_scaling) {
    ...
    KLLM_KERNEL_CHECK(fc1_fp4_act_scale_ != nullptr);
    KLLM_KERNEL_CHECK(fc2_fp4_act_scale_ != nullptr);
  }

  tma_ws_grouped_gemm1_input_ = {};
  tma_ws_grouped_gemm2_input_ = {};
  if (moe_gemm_runner_.supportsTmaWarpSpecialized()) {
    tma_ws_grouped_gemm1_input_.configureWorkspace(getWsPtr(int8_t{}, "tma_ws_gemm1_workspace"), num_experts_per_node,
                                                   getWsPtr(int8_t{}, "gemm_workspace"),
                                                   workspaces.at("gemm_workspace").first, getScalingType());
    tma_ws_grouped_gemm2_input_.configureWorkspace(getWsPtr(int8_t{}, "tma_ws_gemm2_workspace"), num_experts_per_node,
                                                   getWsPtr(int8_t{}, "gemm_workspace"),
                                                   workspaces.at("gemm_workspace").first, getScalingType());
  }
  ...
  if (use_lora) {
    ...
    KLLM_KERNEL_CHECK_WITH_INFO(!use_fp8 || lora_input_ != nullptr, "LoRA input must not be nullptr if FP8 is enabled");
    KLLM_KERNEL_CHECK(lora_fc1_result_ != nullptr);
    KLLM_KERNEL_CHECK(lora_add_bias_ != nullptr);
    KLLM_KERNEL_CHECK(lora_fc2_result_ != nullptr);
  }
...
kernels::fp8_blockscale_gemm::CutlassFp8BlockScaleGemmRunnerInterface* CutlassMoeFCRunner<
    T, WeightType, OutputType, InputType, ScaleBiasType, Enable>::getDeepSeekBlockScaleGemmRunner() const {
  KLLM_KERNEL_CHECK_WITH_INFO((std::is_same_v<T, __nv_bfloat16> && std::is_same_v<OutputType, __nv_bfloat16>),
                              "Block scale GEMM runner only supports BF16 A/output");
  KLLM_KERNEL_CHECK_WITH_INFO((std::is_same_v<WeightType, __nv_fp8_e4m3>),
...
void CutlassMoeFCRunner<T, WeightType, OutputType, InputType, ScaleBiasType, Enable>::BlockScaleFC1(
  ...
  gemm_runner.moeGemm(gemm_output, input, fc1_expert_weights, expert_first_token_offset, num_experts_per_node, shape_n,
                      shape_k, stream, nullptr, quant_params.fp8_block_scaling.fc1_scales_ptrs);

  sync_stream_check_cuda_error(stream);
  constexpr bool bias_is_broadcast = true;
  constexpr bool use_per_expert_act_scale = false;
  doActivation<T, UnfusedGemmOutputType>(output, static_cast<UnfusedGemmOutputType const*>(gemm_output), fc2_fp8_quant,
                                         ...
                                         quant_params, use_per_expert_act_scale, nullptr, stream);
...
void CutlassMoeFCRunner<T, WeightType, OutputType, InputType, ScaleBiasType, Enable>::BlockScaleFC2(
  ...
  // NOTE: we assume gemm_runner.configureWorkspace has already been called.
  gemm_runner.moeGemm(gemm_output, input, fc2_expert_weights, expert_first_token_offset, num_experts_per_node, shape_n,
                      shape_k, stream, nullptr, quant_params.fp8_block_scaling.fc2_scales_ptrs);
  ...
  finalizeMoeRoutingKernelLauncher<OutputType, UnfusedGemmOutputType>(
      ...
      unpermuted_row_to_permuted_row, permuted_row_to_unpermuted_row, token_selected_experts, expert_first_token_offset,
      num_rows, hidden_size, unpadded_hidden_size, k, num_experts_per_node, parallelism_config, enable_alltoall,
      stream);
}
...
T const* CutlassMoeFCRunner<T, WeightType, OutputType, InputType, ScaleBiasType, Enable>::applyPrequantScale(
  ...
  if (use_prequant_scale_kernel) {
    KLLM_KERNEL_CHECK_WITH_INFO((!std::is_same_v<T, WeightType>),
                                "Prequant scales are only used for different weight/activation type!");
    if constexpr (!std::is_same_v<T, WeightType>) {
      llm_kernels::nvidia::tensorrt_llm::dev::kernels::apply_per_channel_scale_kernel_launcher<UnfusedGemmOutputType,
                                                                                               T>(
...
void CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::gemm1(
  ...
  if (fp8_blockscale_gemm_runner) {
    KLLM_KERNEL_CHECK(!min_latency_mode);
    Self::BlockScaleFC1(*fp8_blockscale_gemm_runner, input, output, intermediate_result, expert_first_token_offset,
                        fc1_expert_weights, fc1_expert_biases, fc2_fp8_quant, num_rows, expanded_num_rows, hidden_size,
                        inter_size, num_experts_per_node, fc1_activation_type, quant_params, stream);
    return;
  }
  ...
  if (min_latency_mode) {
    KLLM_KERNEL_CHECK_WITH_INFO(using_tma_ws_gemm1, "Only TMA warp specialized GEMM is supported in min latency mode.");
    // TODO: as for bias, need to get the correct expert id according to the active expert global ids
    KLLM_KERNEL_CHECK_WITH_INFO(fc1_expert_biases == nullptr, "Min latency mode does not support bias.");
    KLLM_KERNEL_CHECK(use_fp4);
  }

  if (using_tma_ws_gemm1) {
    KLLM_KERNEL_CHECK(config.is_tma_warp_specialized);
    KLLM_KERNEL_CHECK(!use_ampere_activation_fusion);

    KLLM_KERNEL_CHECK(!use_fp4 || fc1_fp4_act_flat);
    KLLM_KERNEL_CHECK(!use_fp4 || fc2_fp4_act_flat);
    ...
    if (use_w4afp8) {
      alpha_scale_ptr_array =
          computeFP8DequantScale(alpha_scale_ptr_array, num_experts_per_node, quant_params.groupwise.fc1.alpha, stream);
    }
    ...
    doActivation<GatedActOutputType, UnfusedGemmOutputType>(
        reinterpret_cast<GatedActOutputType*>(output), static_cast<UnfusedGemmOutputType const*>(gemm_output),
        fc2_fp8_quant, fc1_expert_biases, bias_is_broadcast, expert_first_token_offset, num_experts_per_node,
        inter_size, expanded_num_rows, fc1_activation_type, quant_params, use_per_expert_act_scale, fc2_fp4_act_flat,
        stream);
    ...
    KLLM_KERNEL_CHECK(!use_ampere_activation_fusion);
    KLLM_KERNEL_CHECK(!config.is_tma_warp_specialized);
    KLLM_KERNEL_CHECK(!use_block_scaling);
    ...
    doActivation<T, UnfusedGemmOutputType>(
        output, static_cast<UnfusedGemmOutputType const*>(intermediate_result), fc2_fp8_quant, fc1_expert_biases,
    ...
    if (!use_ampere_activation_fusion) {
      ...
      doGatedActivation<GatedActOutputType, UnfusedGemmOutputType>(
          reinterpret_cast<GatedActOutputType*>(output), static_cast<UnfusedGemmOutputType const*>(intermediate_result),
          expert_first_token_offset, inter_size, expanded_num_rows, num_experts_per_node, fc1_activation_type, stream);
...
void CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::gemm2(
  ...
  if (min_latency_mode) {
    KLLM_KERNEL_CHECK_WITH_INFO(using_tma_ws_gemm2, "Only TMA warp specialized GEMM is supported in min latency mode.");
    KLLM_KERNEL_CHECK(use_fp4);
  }
  ...
  if (fp8_blockscale_gemm_runner) {
    Self::BlockScaleFC2(*fp8_blockscale_gemm_runner, input, gemm_output, final_output, expert_first_token_offset,
                        ...
                        permuted_row_to_unpermuted_row, token_selected_experts, num_valid_tokens_ptr, num_rows,
                        expanded_num_rows, hidden_size, unpadded_hidden_size, inter_size, num_experts_per_node, k,
  ...
  if (use_w4afp8) {
    alpha_scale_ptr_array =
        computeFP8DequantScale(alpha_scale_ptr_array, num_experts_per_node, quant_params.groupwise.fc2.alpha, stream);
  }
  ...
  auto universal_input = GroupedGemmInput<T, WeightType, OutputType, OutputType>{
      ...
      quant_params.groupwise.group_size > 0
          ...
          : fc2_int_scales,
      quant_params.groupwise.group_size > 0 ? static_cast<ScaleBiasType const*>(quant_params.groupwise.fc2.weight_zeros)
                                            : nullptr,
  ...
  if (use_lora && !fuse_lora_bias) {
    ...
    loraBiasApplyFunc(
        static_cast<UnfusedGemmOutputType*>(gemm_output), static_cast<UnfusedGemmOutputType const*>(gemm_output),
        nullptr, static_cast<ScaleBiasType const*>(fc2_lora), false, expert_first_token_offset, num_experts_per_node,
...
void CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::runMoe(
  ...
  auto const* input_sf =
      input_sf_void ? reinterpret_cast<TmaWarpSpecializedGroupedGemmInput::ElementSF const*>(input_sf_void) : nullptr;
  ...
  KLLM_KERNEL_CHECK(token_selected_experts);
  ...
  KLLM_KERNEL_CHECK(unpermuted_row_to_permuted_row);
  KLLM_KERNEL_CHECK(full_num_experts % parallelism_config.ep_size == 0);
  KLLM_KERNEL_CHECK(full_num_experts % parallelism_config.cluster_size == 0);

  if (quant_params.mxfp8_mxfp4.fc1.weight_block_scale) {
    KLLM_KERNEL_CHECK_WITH_INFO(
        hidden_size % (64 * 8 / sizeof_bits<WeightType>::value) == 0,
        "Hidden size %d does not meet minimum alignment requirements for MXFP8_MXFP4 MOE GEMM %d", (int)hidden_size,
    ...
    KLLM_KERNEL_CHECK_WITH_INFO(hidden_size % (128 / sizeof_bits<WeightType>::value) == 0,
    ...
    KLLM_KERNEL_CHECK_WITH_INFO(inter_size % (128 / sizeof_bits<WeightType>::value) == 0,
  ...
  KLLM_KERNEL_CHECK_WITH_INFO(num_rows * full_num_experts <= std::numeric_limits<int>::max(),
                              "Number of rows * num_experts is too large");
  KLLM_KERNEL_CHECK_WITH_INFO(experts_per_token * full_num_experts <= std::numeric_limits<int>::max(),
  ...
  KLLM_KERNEL_CHECK_WITH_INFO(!use_lora || !act_fp4, "MOE does not support LoRA with FP4 model");
  ...
  if (int_scales_required) {
    ...
    KLLM_KERNEL_CHECK_WITH_INFO(input_fp8_dequant != nullptr,
  ...
  configureWsPtrs(workspace_ptr, num_rows, hidden_size, inter_size, num_experts_per_node, experts_per_token,
                  fc1_activation_type, parallelism_config, use_lora, use_deepseek_fp8_block_scale, min_latency_mode,
                  use_awq);
  ...
  if (min_latency_mode) {
    KLLM_KERNEL_CHECK(use_lora == false);
    KLLM_KERNEL_CHECK(use_awq == false);
    KLLM_KERNEL_CHECK(use_fp4 == true);

    buildMinLatencyActiveExpertMaps(
        min_latency_params.num_active_experts_per_node, min_latency_params.experts_to_token_score,
        min_latency_params.active_expert_global_ids, expert_first_token_offset_, token_selected_experts,
        token_final_scales, num_rows, experts_per_token, start_expert, end_expert, num_experts_per_node,
        parallelism_config.cluster_rank, parallelism_config.cluster_size, full_num_experts, stream);
    sync_stream_check_cuda_error(stream);

    auto [gemm1_tma_ws_input, gemm2_tma_ws_input] = setupTmaWarpSpecializedInputs(
        num_rows, expanded_num_rows, fc1_activation_type, hidden_size, unpadded_hidden_size, inter_size,
        num_experts_per_node, input_activations_void, input_sf, final_output, fc1_expert_weights, fc2_expert_weights,
        quant_params, fc1_expert_biases, fc2_expert_biases, min_latency_mode, min_latency_params, use_lora,
    ...
    // NOTE(jinxcwu) 经过压测是正常可用的
    // WAR: fusedBuildExpertMapsSortFirstToken kernel will lead to illegal memory access for W4AFP8
    fused_prologue_result =
        fusedBuildExpertMapsSortFirstToken(token_selected_experts, permuted_row_to_unpermuted_row_,
                                           unpermuted_row_to_permuted_row, expert_first_token_offset_, num_rows,
    ...
    if (use_lora) {
      ...
      TLLM_CUDA_CHECK(llm_kernels::nvidia::tensorrt_llm::dev::common::cudaMemcpyAsyncSanitized(
          host_permuted_rows.data(), permuted_row_to_unpermuted_row_, expanded_num_rows * sizeof(int),
    ...
    sync_stream_check_cuda_error(stream);
    Self::gemm1(
        moe_gemm_runner_, blockscale_gemm_runner, gemm1_input, reinterpret_cast<T*>(smoothed_act_), glu_inter_result_,
        expert_first_token_offset_, gemm1_tma_ws_input, fc1_expert_weights, fc1_expert_biases, num_valid_tokens_ptr,
        fc1_int_scales, fc1_fp8_dequant, use_wfp4afp8 ? fc2_wfp4afp8_quant_scale : fc2_fp8_quant, fc1_fp4_act_scale_,
        fc2_fp4_act_scale_, quant_params, num_rows, expanded_num_rows, hidden_size, inter_size, num_experts_per_node,
        fc1_activation_type, alpha_scale_ptr_array_fc1_, !use_lora, stream, *gemm1_config_, false, nullptr, nullptr);
    sync_stream_check_cuda_error(stream);

    auto gemm2_input = reinterpret_cast<T const*>(smoothed_act_);
    sync_stream_check_cuda_error(stream);
...
CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::computeStridesTmaWarpSpecialized(
  ...
  cudaLaunchKernelEx(&config, kernel_instance, expert_first_token_offset, layout_info1, layout_info2, num_tokens,
                     expanded_num_tokens, gemm1_n, gemm1_k, gemm2_n, gemm2_k, num_experts_per_node, gemm1_in, gemm2_in,
                     weights1, weights2, alpha_scale_flat1, alpha_scale_flat2, fp4_act_flat1, fp4_act_flat2,
...
CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::
    computeStridesTmaWarpSpecializedLowLatency(
        ...
        T const* input1, T const* input2, WeightType const* weights1, WeightType const* weights2,
  ...
  KLLM_KERNEL_THROW("Min latency mode is no longer supported");
}
...
CutlassMoeFCRunner<T, WeightType, OutputType, InputType, BackBoneType, Enable>::setupTmaWarpSpecializedInputs(
  ...
  if (min_latency_mode) {
    ...
    KLLM_KERNEL_CHECK_WITH_INFO(gemm1_input != gemm1_output, "Input and output buffers are overlapping");
    return Self::computeStridesTmaWarpSpecializedLowLatency(
        gemm1_tma_ws_input, gemm2_tma_ws_input, num_rows, fc1_out_size, hidden_size, hidden_size, inter_size,
        num_experts_per_node, reinterpret_cast<T const*>(gemm1_input), reinterpret_cast<T const*>(gemm2_input),
        fc1_expert_weights, fc2_expert_weights, quant_params.fp8.dequant_fc1, quant_params.fp8.dequant_fc2, input_sf,
    ...
    KLLM_KERNEL_CHECK_WITH_INFO((gemm1_tma_ws_input.swap_ab && gemm2_tma_ws_input.swap_ab) || !use_w4_groupwise,
    ...
    bool gemm2_using_finalize_fusion =
        gemm2_config_->epilogue_fusion_type == cutlass_extensions::CutlassGemmConfig::EpilogueFusionType::FINALIZE;
    bool using_fused_finalize = use_fused_finalize_ && gemm2_using_finalize_fusion && !use_w4_groupwise && !use_lora;
    KLLM_KERNEL_CHECK_WITH_INFO(using_fused_finalize == gemm2_using_finalize_fusion,
    ...
    if (quant_params.fp8_mxfp4.fc1.weight_block_scale) {
      ...
      KLLM_KERNEL_CHECK(quant_params.fp8_mxfp4.fc2.weight_block_scale);
      KLLM_KERNEL_CHECK(fc1_fp4_act_scale_ != nullptr);
      KLLM_KERNEL_CHECK_WITH_INFO(fc1_fp4_act_scale_ == fc2_fp4_act_scale_,
    ...
    return Self::computeStridesTmaWarpSpecialized(
        expert_first_token_offset_, gemm1_tma_ws_input, gemm2_tma_ws_input, num_rows, expanded_num_rows, fc1_out_size,
        hidden_size, hidden_size, inter_size, num_experts_per_node, reinterpret_cast<T const*>(gemm1_input),
        reinterpret_cast<T const*>(gemm2_input), fc1_expert_weights, fc2_expert_weights, quant_params.fp8.dequant_fc1,
        quant_params.fp8.dequant_fc2, fc1_fp4_act_scale_, fc2_fp4_act_scale_, quant_params, fc1_expert_biases, fc2_bias,
        reinterpret_cast<UnfusedGemmOutputType*>(gemm1_output), reinterpret_cast<UnfusedGemmOutputType*>(fc2_result_),
        permuted_token_final_scales_, permuted_row_to_unpermuted_row_, stream);
  }
}
...
__global__ void prepareMinLatencyBuffer(int* num_active_experts_per_node, int* active_expert_global_ids,
                                        ...
                                        int const num_experts_per_token, int const num_experts_per_node) {
  int tid = threadIdx.x;
  int bid = blockIdx.x;
...
void populateRandomBuffer(void* buffer_void, size_t size, cudaStream_t stream) {
  ...
  KLLM_KERNEL_CHECK_WITH_INFO(size % 128 == 0, "Unexpected size alignment");
  auto threads = size / 128;
  populateRandomBufferKernel<<<ceilDiv(threads, 128), 128, 0, stream>>>(buffer_void, threads);
}

std::map<std::string, std::pair<size_t, size_t>> GemmProfilerBackend::getProfilerWorkspaces(int maxM,
                                                                                            bool is_tma_ws_input) {
  size_t k = mK;
  size_t num_expanded_tokens = mMinLatencyMode ? maxM * mNumExpertsPerNode : maxM * k;

  KLLM_KERNEL_CHECK(mDType != nvinfer1::DataType::kINT4);
  ...
  bool is_4bit_weight =
      mWType == nvinfer1::DataType::kINT4 || mWType == nvinfer1::DataType::kFP4 || mWType == nvinfer1::DataType::kINT64;
  KLLM_KERNEL_CHECK_WITH_INFO(!is_4bit_act || is_4bit_weight, "Cannot have 4-bit activation with non-4-bit weight");
  ...
  bool is_int_w_quant = (mWType == nvinfer1::DataType::kINT8 || mWType == nvinfer1::DataType::kINT4) && mGroupSize <= 0;
  ...
  int64_t const num_blocks_per_seq =
      llm_kernels::nvidia::tensorrt_llm::dev::common::ceilDiv(maxM, num_tokens_per_block);
  size_t const blocked_expert_counts_size =
...
#undef GET_WS_PTR
  ...
  if (mMinLatencyMode) {
    ...
    KLLM_KERNEL_CHECK_WITH_INFO(mNumExpertsPerNode <= 256, "Min latency mode only supports #experts < 256");
    ...
    prepareFakeRouterBuffers<<<grid_dim, num_threads, 0, stream>>>(token_selected_experts_base, num_tokens, mK,
                                                                   mNumExperts);
    sync_stream_check_cuda_error(stream);
    ...
    for (int64_t i = 0; i < NUM_ROUTING_SAMPLES; i++) {
      ...
      threeStepBuildExpertMapsSortFirstToken(token_selected_experts, permuted_token_selected_experts,
                                             ...
                                             expert_first_token_offset, blocked_expert_counts,
...
#undef GET_WS_PTR
  ...
  if ((mWType == nvinfer1::DataType::kINT8 || mWType == nvinfer1::DataType::kINT4 ||
       mWType == nvinfer1::DataType::kUINT8) &&
      mGroupSize < 0) {
    KLLM_KERNEL_CHECK(quant_1 && quant_2);
    ...
    KLLM_KERNEL_CHECK(quant_1 && quant_2 && quant_3 && quant_4 && quant_5 && quant_6);
    mQuantParams =
        QuantParams::FP8MXFP4(static_cast<float const*>(quant_1),
...
#define GET_WS_PTR(type, name)                                                                                        \
  ...
  KLLM_KERNEL_CHECK(mNeedWeights == (expert_weights == nullptr));
      ...
      if (mMinLatencyMode) {
        std::tie(gemm1_tma_ws_input, gemm2_tma_ws_input) =
            mInterface->computeStridesTmaWarpSpecializedLowLatencyDispatch(
                gemm1_tma_ws_input, gemm2_tma_ws_input, num_tokens, fc1_output_size, mExpertHiddenSize,
                mExpertHiddenSize, mExpertInterSize, mNumExpertsPerNode, input, input, weights_sel, weights_sel,
                mQuantParams.fp8.dequant_fc1, mQuantParams.fp8.dequant_fc2, fp4_act_scale_flat, fp4_act_scale_flat,
                mQuantParams, nullptr, nullptr, intermediate, intermediate, num_active_experts_per_node,
        ...
        std::tie(gemm1_tma_ws_input, gemm2_tma_ws_input) = mInterface->computeStridesTmaWarpSpecializedDispatch(
            ...
            fc1_output_size, mExpertHiddenSize, mExpertHiddenSize, mExpertInterSize, mNumExpertsPerNode, input, input,
            weights_sel, weights_sel, mQuantParams.fp8.dequant_fc1, mQuantParams.fp8.dequant_fc2, fp4_act_scale_flat,
...
#undef GET_WS_PTR
  ...
  if (tactic.is_tma_warp_specialized) {
    ...
    KLLM_KERNEL_CHECK_WITH_INFO(tma_ws_input_template.isValid(), "TMA WS input template is not initialized");
  }
  ...
  if (mGemmToProfile == GemmToProfile::GEMM_1) {
    ...
    KLLM_KERNEL_CHECK(mGemmToProfile == GemmToProfile::GEMM_2);
