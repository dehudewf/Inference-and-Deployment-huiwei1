...
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_extensions/compute_occupancy.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_extensions/epilogue_helpers.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_extensions/gemm/kernel/default_fpA_intB_traits.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_extensions/gemm/kernel/moe_cutlass_kernel.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_extensions/gemm/threadblock/default_mma.h"
...
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/common/logger.h"
...
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_kernels/moe_gemm/launchers/fused_moe_gemm_launcher_sm80.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_kernels/moe_gemm/launchers/moe_gemm_tma_ws_launcher.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_kernels/moe_gemm/launchers/moe_gemm_tma_ws_mixed_input_launcher.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_kernels/moe_gemm/moe_gemm_template_dispatch_tma_ws.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_kernels/moe_gemm/moe_gemm_template_dispatch_tma_ws_mixed_dtype.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_kernels/moe_gemm/moe_tma_warp_specialized_traits.h"
...
struct genericMoeGemmKernelLauncher {
  static void call(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs, int sm_count_) {
    ...
    if (!inputs.use_fused_moe) {
      ...
      using EpilogueOp = typename llm_kernels::nvidia::tensorrt_llm::dev::cutlass_extensions::Epilogue<
          CutlassGemmOutputType, MixedGemmArchTraits::ElementsPerAccessC, ElementAccumulator, EpilogueTag>::Op;
      ...
      if constexpr ((std::is_same_v<T, __nv_fp8_e4m3> ||
        ...
        if constexpr (std::is_same_v<T, WeightType>) {
          ...
          KLLM_KERNEL_CHECK_WITH_INFO(inputs.alpha_scales, "alpha_scale_ptr_array shouldn't be nullptr for FP8 Ada.");
        }
      ...
      using GemmKernel_ = typename cutlass::gemm::kernel::DefaultGemmGrouped<
          ...
          cutlass::ComplexTransform::kNone, MixedGemmArchTraits::ElementsPerAccessB, CutlassGemmOutputType,
          cutlass::layout::RowMajor, ElementAccumulator, typename MixedGemmArchTraits::OperatorClass, arch,
          ThreadblockShape, WarpShape, typename MixedGemmArchTraits::InstructionShape, EpilogueOp,
      ...
      if (inputs.occupancy != nullptr) {
        *inputs.occupancy =
            llm_kernels::nvidia::tensorrt_llm::dev::cutlass_extensions::compute_occupancy_for_kernel<GemmKernel>();
        return;
      }
      int occupancy = std::min(2, GemmGrouped::maximum_active_blocks());
      KLLM_KERNEL_CHECK_WITH_INFO(occupancy > 0, "GPU lacks the shared memory resources to run GroupedGEMM kernel");
      ...
      typename GemmGrouped::Arguments args(
          ...
          reinterpret_cast<CutlassGemmOutputType*>(inputs.C), inputs.total_tokens_including_expert, inputs.n, inputs.k);
      ...
      KLLM_KERNEL_CHECK_WITH_INFO(
          ...
          "Failed to run cutlass grouped gemm. Error: " + std::string(cutlassGetStatusString(run_status)));
                         ...
                         (std::is_same_v<EpilogueTag, cutlass_extensions::EpilogueOpDefaultSilu> ||
                          std::is_same_v<EpilogueTag, cutlass_extensions::EpilogueOpDefaultFtGelu>))  // use fused moe
                                                                                                      // gemm kernel..
                                                                                                      // (only support
                                                                                                      // fp16 or bf16)
    {
      sm80_generic_fused_moe_gemm_kernelLauncher<ElementType, CutlassWeightType, ThreadblockShape::kM,
          ...
          reinterpret_cast<ElementType*>(inputs.C), inputs.total_tokens_including_expert, inputs.num_rows, inputs.n,
          inputs.k, inputs.num_experts, sm_count_, inputs.stream, inputs.occupancy);
    }
  }
};
...
struct genericMoeGemmKernelLauncher<__nv_bfloat16, __nv_fp8_e4m3, GemmOutputType, arch, QuantOp, EpilogueTag,
  ...
  static void call(GroupedGemmInput<__nv_bfloat16, __nv_fp8_e4m3, GemmOutputType, GemmOutputType> inputs,
                   int sm_count_) {}
};
...
static void dispatch(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs, int sm_count_) {
  ...
  if constexpr ((Stages == 2 || Arch::kMinComputeCapability >= 80) &&
    ...
    auto* launcher =
        kernels::cutlass_kernels::genericMoeGemmKernelLauncher<T, WeightType, GemmOutputType, Arch,
    ...
    if (!std::is_same_v<WeightType, T> && inputs.groupwise_quant_group_size > 0) {
      launcher = inputs.zeros
                     ? kernels::cutlass_kernels::genericMoeGemmKernelLauncher<
                           T, WeightType, GemmOutputType, Arch, cutlass::WeightOnlyQuantOp::FINEGRAINED_SCALE_AND_ZEROS,
                           EpilogueTag, ThreadblockShape, WarpShape, Stages>::call
                     : kernels::cutlass_kernels::genericMoeGemmKernelLauncher<
                           T, WeightType, GemmOutputType, Arch, cutlass::WeightOnlyQuantOp::FINEGRAINED_SCALE_ONLY,
                           EpilogueTag, ThreadblockShape, WarpShape, Stages>::call;
    }
    launcher(inputs, sm_count_);
  } else {
    KLLM_KERNEL_THROW("Cutlass gemm. Not instantiated for arch %d with stages set to %d", Arch::kMinComputeCapability, Stages);
  }
}
...
void dispatchGemmConfig(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs, int sm_count_) {
  switch (inputs.gemm_config.stages) {
    ...
    case 4:
      ...
      break;
    default:
      KLLM_KERNEL_THROW("dispatchGemmConfig does not support stages %d", inputs.gemm_config.stages);
      break;
  }
}

template <
    typename T, typename WeightType, typename GemmOutputType, typename arch, typename EpilogueTag,
    typename ThreadblockShape, typename WarpShape,
    typename std::enable_if<((!std::is_same<T, __nv_fp8_e4m3>::value) && (!std::is_same<T, __nv_fp8_e5m2>::value)) ||
                            std::is_same<T, WeightType>::value>::type* = nullptr>
void dispatchGemmConfig(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs, int sm_count_) {
...
void dispatchMoeGemmToCutlass(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs, int sm_count_) {
  switch (inputs.gemm_config.tile_config_sm80) {
    ...
    case cutlass_extensions::CutlassTileConfig::Undefined:
      KLLM_KERNEL_THROW("GEMM config undefined.");
      break;
    case cutlass_extensions::CutlassTileConfig::ChooseWithHeuristic:
      KLLM_KERNEL_THROW("GEMM config should have already been set by heuristic.");
      break;
    default:
      KLLM_KERNEL_THROW("Config is invalid for same type tensorop GEMM.");
      break;
  }
}
...
void dispatchMoeGemmToCutlass(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs, int sm_count_) {
  ...
  switch (inputs.gemm_config.tile_config_sm80) {
    ...
    case cutlass_extensions::CutlassTileConfig::ChooseWithHeuristic:
      ...
      break;
    default:
      KLLM_KERNEL_THROW("Config is invalid for mixed type tensorop GEMM.");
      break;
  }
}
...
void dispatchMoeGemmToCutlass(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs, int sm_count_) {
  switch (inputs.gemm_config.tile_config_sm80) {
    ...
    case cutlass_extensions::CutlassTileConfig::ChooseWithHeuristic:
      ...
      break;
    default:
      KLLM_KERNEL_THROW("Unsupported config for float MoE gemm.");
      break;
  }
}
...
MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::getAmpereConfigs(int sm) {
  using llm_kernels::nvidia::tensorrt_llm::dev::cutlass_extensions::CutlassGemmConfig;
...
MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::getTmaWarpSpecializedConfigs(int sm,
  ...
  auto config_type_param = static_cast<CutlassGemmConfig::CandidateConfigTypeParam>(
      weight_only_flag | simt_only_flag | grouped_gemm_flag | enable_blackwell | enable_hopper | fp8_only_flag |
      fp4_only_flag);
  KLLM_KERNEL_CHECK_WITH_INFO(!(enable_blackwell && enable_hopper), "Blackwell and hopper flags are mutually exclusive");
  ...
  if ((sm == 120 || sm == 121) && !kernels::cutlass_kernels::isValidSM120MOESpecialisation<T, WeightType>()) {
    TLLM_LOG_TRACE("Blackwell SM120 is not supported for this configuration, not selecting any TMA WS implementations");
    return {};
  }
  ...
  std::transform(swap_ab_configs.begin(), swap_ab_configs.end(), std::back_inserter(tma_ws_configs), [](auto& config) {
    KLLM_KERNEL_CHECK_WITH_INFO(!config.swap_ab, "Swap AB is already set");
    config.swap_ab = true;
    return config;
  });
...
MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::MoeGemmRunner() {
  int device{-1};
  llm_kernels::nvidia::tensorrt_llm::dev::common::check_cuda_error(cudaGetDevice(&device));
  sm_ = llm_kernels::nvidia::tensorrt_llm::dev::common::getSMVersion();
  llm_kernels::nvidia::tensorrt_llm::dev::common::check_cuda_error(
...
void MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::dispatchToArch(
  ...
  KLLM_KERNEL_CHECK_WITH_INFO(sm_ >= 89 || !hopper_inputs.isValid(),
                       "Hopper input information is set for non specialized implementation");
  KLLM_KERNEL_CHECK_WITH_INFO(sm_ >= 90 || !inputs.gemm_config.is_tma_warp_specialized,
                       "Hopper configuration provided for non-Hopper architecture");

  if (sm_ >= 75 && sm_ < 80) {
    if constexpr (!std::is_same_v<WeightType, __nv_fp4_e2m1>) {
      dispatchMoeGemmToCutlass<T, WeightType, ScaleBiasType, cutlass::arch::Sm75, EpilogueTag>(inputs,
                                                                                               multi_processor_count_);
    } else {
      KLLM_KERNEL_THROW("FP4 data type is not supported on SM < 90");
    }
    ...
    if constexpr (!std::is_same_v<WeightType, __nv_fp4_e2m1>) {
      if constexpr (use_fp8 || use_w4afp8) {
        ...
        KLLM_KERNEL_CHECK_WITH_INFO(sm_ == 89, "For sm >= 80 and < 90, fp8 is only supported with sm == 89");
    ...
    if constexpr (kernels::cutlass_kernels::isValidTmaWarpSpecializedMOESpecialisation<T, WeightType, EpilogueTag>() &&
      ...
      if (inputs.gemm_config.sm_version >= 90) {
        KLLM_KERNEL_CHECK_WITH_INFO(inputs.gemm_config.sm_version == sm_, "Using SM %d configuration for SM %d device",
                             inputs.gemm_config.sm_version, sm_);
        KLLM_KERNEL_CHECK_WITH_INFO(inputs.biases != nullptr || hopper_inputs.ptr_c == nullptr,
                             "Input biases and hopper input disagree if bias is enabled");
        KLLM_KERNEL_CHECK_WITH_INFO(hopper_inputs.isValid(),
          ...
          switch (hopper_inputs.fusion) {
            case TmaWarpSpecializedGroupedGemmInput::EpilogueFusion::FINALIZE:
              return &dispatchMoeGemmSelectTileShapeTmaWarpSpecialized<
                  T, WeightType, OutputType, EpilogueTag, TmaWarpSpecializedGroupedGemmInput::EpilogueFusion::FINALIZE>;
            case TmaWarpSpecializedGroupedGemmInput::EpilogueFusion::NONE:
              return &dispatchMoeGemmSelectTileShapeTmaWarpSpecialized<
                  T, WeightType, OutputType, EpilogueTag, TmaWarpSpecializedGroupedGemmInput::EpilogueFusion::NONE>;
            ...
            default:
              KLLM_KERNEL_THROW("Unimplemented fusion %d requested", (int)hopper_inputs.fusion);
          };
        };
        auto selected_func = select_function();
        selected_func(hopper_inputs, inputs.num_experts, inputs.gemm_config, multi_processor_count_, inputs.stream,
    ...
    if constexpr (use_w4afp8) {
      KLLM_KERNEL_CHECK_WITH_INFO(inputs.gemm_config.is_tma_warp_specialized,
      ...
      if (inputs.k % 512 == 0) {
        ...
        KLLM_KERNEL_THROW("Invalid GEMM K size %d", (int)inputs.k);
      }
      return;
    }
    ...
    if constexpr (use_wfp4a16) {
      ...
      // EpilogueTag is ignored
      sm90_dispatch_moe_mixed_dtype_gemm_to_cutlass<T, WeightType, ScaleBiasType, cutlass_extensions::EpilogueOpDefault,
    ...
    if constexpr (kernels::cutlass_kernels::isValidAmpereMOESpecialisation<T, WeightType, EpilogueTag>()) {
      KLLM_KERNEL_CHECK_WITH_INFO(!use_fp8, "No fallback FP8 implementation available");
      KLLM_KERNEL_CHECK_WITH_INFO(
          use_w4afp8 || !hopper_inputs.isValid(),
      ...
      KLLM_KERNEL_CHECK_WITH_INFO(inputs.gemm_config.sm_version == 80,
      ...
      KLLM_KERNEL_THROW("Configuration expects SM80 but configuration is not supported by SM80 kernels");
    }
  } else {
    KLLM_KERNEL_THROW("Arch unsupported for MoE GEMM");
  }
}
...
size_t MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::calcMaxWorkspaceSize(int num_experts) const {
  if constexpr (use_w4_groupwise) {
    return calcMaxWorkspaceSizeTmaWarpSpecializedMixedInput<T, WeightType, OutputType>(num_experts,
                                                                                       multi_processor_count_);
  }
...
#define CALC_SIZE_FUSION(FUSION)                                                                     \
  do {                                                                                               \
    try {                                                                                            \
      ...
      has_config = true;                                                                             \
    } catch (llm_kernels::nvidia::tensorrt_llm::dev::common::TllmException const& e) {               \
...
#undef CALC_SIZE_FUSION
    ...
    KLLM_KERNEL_CHECK_WITH_INFO(has_config, "Could not find valid config when calculating workspace size");
    return max_size;
  } else {
    KLLM_KERNEL_THROW("Attempting to calculate Hopper GEMM workspace size with unsupported weight combination");
    return 0;
  }
}
...
void MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::moeGemmBiasAct(
  ...
  switch (inputs.activation_type) {
    ...
    case ActivationType::InvalidType:
      KLLM_KERNEL_THROW("Activation type for fpA_intB must be valid.");
      break;
    default:
      KLLM_KERNEL_THROW("Invalid activation type.");
      break;
  }
}
