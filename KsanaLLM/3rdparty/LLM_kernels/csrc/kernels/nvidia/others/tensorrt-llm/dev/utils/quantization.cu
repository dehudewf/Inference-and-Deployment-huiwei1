...
#include <float.h>
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/common/cudaTypeUtils.cuh"
...
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/common/quantTypeUtils.cuh"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/common/reduceKernelUtils.cuh"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/utils/quantization.cuh"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/utils/quantization.h"
...
template <typename T>
void invokeQuantization(int8_t* dst, T const* src, int64_t const size, float const* scalePtr, cudaStream_t stream,
                        int maxGridSize) {
  KLLM_KERNEL_CHECK_WITH_INFO(size % 4 == 0, "[ERROR][invokeQuantization] size should be a multiple of 4.\n");

  int numBlocks{static_cast<int>((size + 255) / 256)};
  dim3 grid(std::min(numBlocks, maxGridSize));
  KLLM_KERNEL_CHECK_WITH_INFO(grid.x <= static_cast<unsigned int>(maxGridSize),
...
template void invokeQuantization<float>(int8_t* dst, float const* src, int64_t const size, float const* scalePtr,
                                        cudaStream_t stream, int maxGridSize);

template void invokeQuantization<half>(int8_t* dst, half const* src, int64_t const size, float const* scalePtr,
                                       cudaStream_t stream, int maxGridSize);
...
void invokePerTokenQuantization(QuantT* dst, T const* src, int64_t const numRows, int64_t const numCols,
                                float const* clampPtr, float* scalePtr, float* sumPtr, QuantMode quantMode,
                                cudaStream_t stream) {
  ...
  KLLM_KERNEL_CHECK_WITH_INFO(numCols % NUM_ELTS_PER_VEC == 0, "Not supported.");
  ...
  if (dynamicSmemSz >= 48 * 1024) {
    cudaError_t res = cudaFuncSetAttribute(perTokenQuantization<T, QuantT, true>,
  ...
  // Do we use smem ?
  if (useSmem) {
    perTokenQuantization<T, QuantT, true><<<grid, block, dynamicSmemSz, stream>>>(dst, src, numRows, numCols, clampPtr,
                                                                                  scalePtr, sumPtr, hasFp8MinScaling);
  } else {
...
void invokeFP4Quantization(int b, int m, int n, T const* input, float const* SFScale, int64_t* output, int32_t* SFOuput,
  ...
  if constexpr (std::is_same_v<T, __nv_fp8_e4m3>) {
    ...
    auto* kernel_instance =
        useUE8M0 ? &quantize_with_block_size<BlockScaleQuantizationType::FP8_TO_FP4, T, SF_VEC_SIZE, true>
...
#endif
  {
    ...
    // Launch the cvt kernel.
    auto* kernel_instance =
        useUE8M0 ? &quantize_with_block_size<BlockScaleQuantizationType::FP16_TO_FP4, T, SF_VEC_SIZE, true>
    ...
    attrs[0].val.programmaticStreamSerializationAllowed = llm_kernels::nvidia::tensorrt_llm::dev::common::getEnvEnablePDL();
    config.numAttrs = 1;
    config.attrs = attrs;
...
void invokeMxFP8Quantization(int b, int m, int n, int padded_n, T const* input, int64_t* output, int32_t* SFOuput,
  ...
  config.numAttrs = 1;
  config.attrs = attrs;
  cudaLaunchKernelEx(&config, quantize_with_block_size<BlockScaleQuantizationType::FP16_TO_MXFP8, T, SF_VEC_SIZE, true>,
                     b, m, n, padded_n, input, nullptr, reinterpret_cast<uint32_t*>(output),
                     reinterpret_cast<uint32_t*>(SFOuput), layout);
}
...
__global__ void block_scale_interleave_reverse_kernel(int numBatches, int numRows, int numCols, uint8_t const* SFIn,
                                                      uint8_t* SFOutput) {
...
__device__ float getMaxAbs(float4& vec) {
  ...
  llm_kernels::nvidia::tensorrt_llm::dev::common::blockReduceMaxV2<float, 1>(&absMaxVal);
  return absMaxVal;
}

template <typename T>
__global__ void computePerTokenGlobalScaleForFP4QuantizationKernel(int b, int m, int n, T const* input,
                                                                   int const* tokensPerBatch, float* globalScale) {
...
void computePerTokenGlobalScaleForFP4Quantization(int b, int m, int n, T const* input, int const* tokensPerBatch,
  ...
  static constexpr int ElemsPerVec = 16 / sizeof(T);
  KLLM_KERNEL_CHECK(n % (ElemsPerVec * 32) == 0 and b > 0);
  ...
  config.numAttrs = 1;
  config.attrs = attrs;
  TLLM_CUDA_CHECK(cudaLaunchKernelEx(&config, computePerTokenGlobalScaleForFP4QuantizationKernel<T>, b, m, n, input,
                                     tokensPerBatch, globalScale));
}
...
template void invokeFP4Quantization<__nv_bfloat16, 16>(int b, int m, int n, __nv_bfloat16 const* input,
                                                       ...
                                                       bool useUE8M0, QuantizationSFLayout layout,
                                                       int multiProcessorCount, cudaStream_t stream);
...
template void invokeMxFP8Quantization<__nv_bfloat16>(int b, int m, int n, int padded_n, __nv_bfloat16 const* input,
                                                     int64_t* output, int32_t* SFOuput, QuantizationSFLayout layout,
                                                     int multiProcessorCount, cudaStream_t stream);
