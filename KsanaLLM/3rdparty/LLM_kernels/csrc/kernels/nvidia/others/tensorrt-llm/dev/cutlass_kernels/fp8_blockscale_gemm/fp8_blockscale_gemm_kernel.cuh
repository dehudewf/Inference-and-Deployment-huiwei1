...
__inline__ __device__ __nv_bfloat16 warpReduceSum(__nv_bfloat16 val) {
  constexpr uint32_t FINAL_MASK = 0xffffffff;
#pragma unroll
  for (int mask = 16; mask > 0; mask >>= 1) val = __hmax(val, __shfl_xor_sync(FINAL_MASK, val, mask, 32));
  return val;
}
...
namespace llm_kernels::nvidia::tensorrt_llm::dev::kernels::fp8_blockscale_gemm {
...
__global__ void __launch_bounds__(TILE_M == 64 ? 256 : 384, 1)
    cooperative_1x128_by_128x128_fp8_gemm_kernel(ElementD* gmem_d, int ld_d, float const* scales_b,
      ...
      for (int k_iter = 0; k_iter < div_up(shape_k, K_PER_ITER); k_iter++) {
        auto copy_func = [&](Barrier& empty_bar, int stage_range_start, int stage_range_end) {
          ...
          for (int i = stage_range_start; i < stage_range_end; i++) {
            ...
            cde::cp_async_bulk_tensor_2d_global_to_shared(smem_a[i], &tensor_map_a, k_idx,
                                                          tile_m_idx * TILE_M + problem_m_offset, bar);
            cde::cp_async_bulk_tensor_2d_global_to_shared(smem_b[i], &tensor_map_b, k_idx,
                                                          tile_n_idx * TILE_N + problem_n_offset, bar);
            if constexpr (std::is_same_v<ProblemVisitor, StridedBatchedGemmProblemVisitor<TILE_M, TILE_N>>) {
              ...
              cde::cp_async_bulk_tensor_2d_global_to_shared(
                  smem_scales_a[i], &tensor_map_scales_a, (problem_m_padded_offset + tile_m_idx * TILE_M) / ScaleGranMA,
    ...
    if constexpr (LayoutD == Layout::RowMajor) {
      ...
      for (int line_idx = warp_idx; line_idx < num_lines; line_idx += num_warps) {
        ...
        for (int elem_idx = lane_idx; elem_idx < int4_per_line; elem_idx += 32) {
          ...
          int4* s_data_addr =
              &smem_c_int4[line_idx * (int4_per_tile_line + SMEM_C_PADDING * sizeof(ElementD) / sizeof(int4)) +
                           elem_idx];
...
template <typename ElementA, Layout LayoutA, typename ElementB, Layout LayoutB, typename ElementD, Layout LayoutD,
          typename ElementAccumulator, typename ElementCompute, typename ElementScalar, int TILE_M, int TILE_N,
          int TILE_K, ScaleType ScaleTypeA, ScaleType ScaleTypeB, int ScaleGranMA = 0, int ScaleGranKA = 0,
          int ScaleGranNB = 0, int ScaleGranKB = 0, int NUM_OF_STAGES = 0>
class Fp8Gemm {
 public:
 ...
 public:
  ...
  static void run(ElementA* gmem_a, ElementB* gmem_b, ElementD* gmem_d, ElementScalar* scales_a,
                  ElementScalar const* scales_b, int num_problems, int64_t const* problem_m_offsets, int shape_n,
                  int shape_k, int max_shape_m, cudaStream_t stream = 0, int guessed_m = TILE_M,
...
#undef Kernel
  ...
  // StridedBatchedGemm
  static void run(ElementA* gmem_a, int ld_a, int stride_a, ElementB* gmem_b, int ld_b, int stride_b, ElementD* gmem_d,
                  int ld_d, int stride_d, ElementScalar* scales_a, int stride_scales_a, ElementScalar const* scales_b,
                  int shape_m, int shape_n, int shape_k, int num_problems, cudaStream_t stream = 0) {
...
__global__ void scale_1x128_kernel(OutputType* output, ScaleType* scales, InputType const* const input, int dim_x,
                                   int dim_y) {
...
__global__ void scale_1x128_kernel(OutputType* output, float* scales, InputType const* input,
  ...
  for (size_t warp_idx = (threadIdx.x + blockIdx.x * blockDim.x) / 32; warp_idx < total_scales;
    ...
    kernel_utils::fast_divmod(scales_idx_y, scales_idx_x, warp_idx, scales_along_dim_x, scale_dim_x_mul,
                              scale_dim_x_shr);
...
template <typename InputType, typename OutputType, typename ScaleType = float>
__global__ void scale_1x128_reshape_kernel(OutputType* output, ScaleType* scales, InputType const* const input,
                                           int dim_x, int dim_h, int dim_y, int stride_x) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 890))
  size_t scales_along_dim_x = div_up(dim_x, 128);
  size_t scales_along_dim_y = div_up(dim_y, 1);
...
template <typename InputType, typename OutputType, typename ScaleType = float>
__global__ void scale_128x128_kernel(OutputType* output, ScaleType* scales, InputType const* const input, int dim_x,
                                     int dim_y) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900))
...
void fp8_1x128_cs(__nv_fp8_e4m3* mat_quant, float* scales, __nv_bfloat16 const* mat, int shape_x, int shape_y,
                  cudaStream_t stream) {
  if (kNumDeviceSMs < 0) {
    kNumDeviceSMs = llm_kernels::nvidia::tensorrt_llm::dev::common::getMultiProcessorCount();
  }
...
void fp8_1x128_cs_reshape(__nv_fp8_e4m3* mat_quant, float* scales, __nv_bfloat16 const* mat, int shape_x, int shape_h,
  ...
  scale_1x128_reshape_kernel<<<kNumDeviceSMs * 8, 256, 0, stream>>>(mat_quant, scales, mat, shape_x, shape_h, shape_y,
                                                                    stride_x);
}

void fp8_128x128_cs(__nv_fp8_e4m3* mat_quant, float* scales, __nv_bfloat16 const* mat, int shape_x, int shape_y,
                    cudaStream_t stream) {
  if (kNumDeviceSMs < 0) {
    kNumDeviceSMs = llm_kernels::nvidia::tensorrt_llm::dev::common::getMultiProcessorCount();
  }
...
#define DISPATCH_BLOCK_SIZE(TILE_M, TILE_N)                                                                            \
  {                                                                                                                    \
    using GemmType = Fp8Gemm<__nv_fp8_e4m3, Layout::RowMajor, __nv_fp8_e4m3, Layout::ColMajor, __nv_bfloat16,          \
                             Layout::RowMajor, float, float, float, TILE_M, TILE_N, 128, ScaleType::PerSubChannel,     \
...
void gemm_dispatch_old(void* mat_a, void* mat_b, void* mat_d, float* scales_a, float* scales_b, int num_problems,
                       int64_t const* problem_m_offsets, int max_shape_m, int shape_n, int shape_k,
                       cudaStream_t stream) {
  if (kNumDeviceSMs < 0) {
    kNumDeviceSMs = llm_kernels::nvidia::tensorrt_llm::dev::common::getMultiProcessorCount();
  }
...
#define DISPATCH_BLOCK_SIZE(TILE_M, TILE_N)                                                                        \
  {                                                                                                                \
    ...
    GemmType::run(reinterpret_cast<__nv_fp8_e4m3*>(mat_a), reinterpret_cast<__nv_fp8_e4m3*>(mat_b),                \
                  ...
                  shape_n, shape_k, max_shape_m, stream                                                            \
                                                                                                                   \
    );                                                                                                             \
  }                                                                                                                \
  break
...
void gemm_dispatch(void* mat_a, int ld_a, void* mat_b, int ld_b, void* mat_d, int ld_d, float* scales_a,
  ...
  if (num_device_sms < 0) {
    num_device_sms = kNumDeviceSMs = llm_kernels::nvidia::tensorrt_llm::dev::common::getMultiProcessorCount();
  }
  ...
  if (shape_m >= m_threshold) {
    ...
    auto runtime =
        deep_gemm::jit::getGlobalCompiler().build(shape_n, shape_k, best_block_m, best_block_n, block_k, num_problems,
                                                  best_num_stages, best_num_tma_multicast, deep_gemm::GemmType::Normal);
    ...
    auto runtime = deep_gemm::jit::getGlobalCompiler().build(shape_n, shape_k, best_block_m, best_block_n, block_k,
                                                             num_problems, best_num_stages, best_num_tma_multicast,
                                                             deep_gemm::GemmType::Normal, true);
    auto kernel = reinterpret_cast<cudaKernel_t>(runtime->getKernel());
...
void gemm_dispatch_sm89(void* mat_a, void* mat_b, void* mat_d, float* scales_a, float* scales_b, uint32_t shape_m,
  ...
  if (kSmemSize > (48 << 10)) {
    ...
    KLLM_KERNEL_CHECK_WITH_INFO(result == cudaSuccess, "sm89 gemm kernel cannot launch: %s",
...
void fp8_gemm_run(__nv_fp8_e4m3* mat_a, int ld_a, __nv_fp8_e4m3* mat_b, int ld_b, __nv_bfloat16* mat_d, int ld_d,
                  uint32_t shape_m, uint32_t shape_n, uint32_t shape_k, float* scales_a, float* scales_b,
                  cudaStream_t stream) {
  if (shape_m == 0) {
    return;
  }
#ifndef PLACEHOLDER_KERNELS
  int arch = llm_kernels::nvidia::tensorrt_llm::dev::common::getSMVersion();
  if (arch == 89) {
...
void fp8_gemm_run(__nv_bfloat16 const* mat_a, __nv_fp8_e4m3* fp8_mat_a, int ld_a, float* scales_a,
                  __nv_bfloat16 const* mat_b, __nv_fp8_e4m3* fp8_mat_b, int ld_b, float* scales_b, __nv_bfloat16* mat_d,
                  int ld_d, uint32_t shape_m, uint32_t shape_n, uint32_t shape_k, cudaStream_t stream,
...
void grouped_gemm_dispatch(__nv_fp8_e4m3* mat_a, __nv_fp8_e4m3* mat_b, __nv_bfloat16* mat_d, uint32_t num_problems,
                           int64_t const* problem_m_offsets, uint32_t expected_m, uint32_t max_shape_m,
                           uint32_t max_shape_m_padded, uint32_t shape_n, uint32_t shape_k, float* scales_a,
  ...
  if (expected_m >= m_per_expert_threshold) {
    ...
    auto [best_block_m, best_block_n, best_num_stages, best_num_tma_multicast, best_smem_size] =
        deep_gemm::jit::get_best_gemm_config(shape_n, expected_m, shape_k, num_problems, num_device_sms, false, true);
    auto runtime = deep_gemm::jit::getGlobalCompiler().build(shape_n, shape_k, best_block_m, best_block_n, block_k,
                                                             num_problems, best_num_stages, best_num_tma_multicast,
                                                             deep_gemm::GemmType::GroupedWithOffset, true);
    auto kernel = reinterpret_cast<cudaKernel_t>(runtime->getKernel());

    deep_gemm::runGemmSwapAB(kernel, mat_b, 0, mat_a, 0, mat_d, 0, scales_b, scales_a, shape_n, max_shape_m, shape_k,
                             best_block_m, best_block_n, block_k, num_problems, best_num_tma_multicast,
                             deep_gemm::GemmType::GroupedWithOffset, const_cast<int64_t*>(problem_m_offsets), stream,
                             num_device_sms, static_cast<uint32_t>(best_smem_size), max_shape_m_padded);
  }
...
void fp8_grouped_gemm_run(__nv_bfloat16 const* mat_a, __nv_fp8_e4m3* fp8_mat_a, float* scales_a,
                          ...
                          int64_t max_shape_m_padded, int shape_n, int shape_k, cudaStream_t stream,
                          bool internal_quantize_a = true, bool internal_quantize_b = true) {
  if (kNumDeviceSMs < 0) {
    kNumDeviceSMs = llm_kernels::nvidia::tensorrt_llm::dev::common::getMultiProcessorCount();
  }
  ...
  if (internal_quantize_a) {
    ...
    cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
    kernel<<<num_blocks, NumThreads, smem_size, stream>>>(fp8_mat_a, scales_a, mat_a, problem_m_offsets, num_problems,
                                                          shape_k, max_shape_m_padded, scale_dim_x_mul,
                                                          scale_dim_x_shr);
  }
  ...
  if (kDeepGemmEnabled) {
    ...
    GemmType::run(fp8_mat_a, fp8_mat_b, mat_d, scales_a, scales_b, num_problems, problem_m_offsets, shape_n, shape_k,
                  static_cast<int>(max_shape_m), stream, 128, static_cast<int>(max_shape_m_padded));
  }
}

void strided_batch_gemm_dispatch(__nv_fp8_e4m3* mat_a, int ld_a, int stride_a, __nv_fp8_e4m3* mat_b, int ld_b,
                                 int stride_b, __nv_bfloat16* mat_d, int ld_d, int stride_d, float* scales_a,
                                 float* scales_b, uint32_t num_problems, uint32_t shape_m, uint32_t shape_n,
  ...
  auto runtime = deep_gemm::jit::getGlobalCompiler().build(shape_n, shape_k, best_block_m, best_block_n, block_k,
                                                           num_problems, best_num_stages, best_num_tma_multicast,
                                                           deep_gemm::GemmType::StridedBatched);
  auto kernel = reinterpret_cast<cudaKernel_t>(runtime->getKernel());
  deep_gemm::runGemm(kernel, mat_a, static_cast<uint64_t>(ld_a), static_cast<uint64_t>(stride_a), mat_b,
                     ...
                     static_cast<uint64_t>(stride_d), scales_a, scales_b, shape_m, shape_n, shape_k, best_block_m,
                     best_block_n, block_k, num_problems, best_num_tma_multicast, deep_gemm::GemmType::StridedBatched,
...
void strided_batch_gemm_dispatch_sm89(__nv_fp8_e4m3* mat_a, int ld_a, int stride_a, __nv_fp8_e4m3* mat_b, int ld_b,
                                      int stride_b, __nv_bfloat16* mat_d, int ld_d, int stride_d, float* scales_a,
                                      int stride_scales_a, float* scales_b, uint32_t num_problems, uint32_t shape_m,
  ...
  ada_blockwise_gemm::sm89_fp8_bmm_1d1d_impl<GemmKernel>
      <<<grid, block, kSmemSize, stream>>>(shape_m, shape_n, shape_k, mat_a, mat_b, mat_d, scales_a, scales_b, stride_a,
                                           stride_b, stride_d, stride_scales_a, stride_scales_b);
}

void fp8_stride_batch_gemm_run(__nv_bfloat16 const* mat_a, __nv_fp8_e4m3* fp8_mat_a, float* scales_a, int ld_a,
                               int stride_a, int stride_scales_a, __nv_bfloat16 const* mat_b, __nv_fp8_e4m3* fp8_mat_b,
                               float* scales_b, int ld_b, int stride_b, __nv_bfloat16* mat_d, int ld_d, int stride_d,
                               uint32_t num_problems, uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
  ...
  if (arch == 89) {
    strided_batch_gemm_dispatch_sm89(fp8_mat_a, ld_a, stride_a, fp8_mat_b, ld_b, stride_b, mat_d, ld_d, stride_d,
                                     scales_a, stride_scales_a, scales_b, num_problems, shape_m, shape_n, shape_k,
                                     stream);
    return;
  }
  if (kDeepGemmEnabled) {
    ...
    GemmType::run(fp8_mat_a, ld_a, stride_a, fp8_mat_b, ld_b, stride_b, mat_d, ld_d, stride_d, scales_a,
                  stride_scales_a, scales_b, static_cast<int>(shape_m), static_cast<int>(shape_n),
                  static_cast<int>(shape_k), static_cast<int>(num_problems), stream);
  }
}
