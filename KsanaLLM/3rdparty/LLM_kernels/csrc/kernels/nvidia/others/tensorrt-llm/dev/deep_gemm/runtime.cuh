...
static bool kJitDebugging = []() {
  char const* env_var = getenv("KLLM_DG_JIT_DEBUG");
  return env_var && (std::string(env_var) == "1" || std::string(env_var) == "true");
}();

static bool kJitUseNvcc = []() {
  // char const* env_var = getenv("KLLM_DG_JIT_USE_NVCC");
  // return env_var && (std::string(env_var) == "1" || std::string(env_var) == "true");
  // NOTE(jinxcwu) 强制使用nvcc
  return true;
}();

static bool kJitDumpCubin = []() {
  char const* env_var = getenv("KLLM_DG_JIT_DUMP_CUBIN");
  return env_var && (std::string(env_var) == "1" || std::string(env_var) == "true");
}();
...
class RuntimeCache {
 public:
  // 获取指定线程的缓存实例
  static RuntimeCache& getInstance(int thread_id = 0) {
    // 确保thread_id在有效范围内
    assert(thread_id >= 0 && thread_id < MAX_THREADS);
    static RuntimeCache instances[MAX_THREADS];
  ...
  void set(std::string const& path, std::unique_ptr<Runtime>&& runtime) { cache_[path] = std::move(runtime); }
