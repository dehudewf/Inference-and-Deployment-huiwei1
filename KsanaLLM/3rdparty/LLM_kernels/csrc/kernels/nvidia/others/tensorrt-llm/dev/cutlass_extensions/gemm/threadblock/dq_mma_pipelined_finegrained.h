...
class DqMmaPipelined<Shape_, IteratorA_, SmemIteratorA_, IteratorB_, SmemIteratorB_, IteratorScale_, SmemIteratorScale_,
 ...
 public:
  ...
  void copy_scales_and_advance(IteratorScale& iterator_scale) {
    using TransformScale = NumericArrayConverter<typename SmemIteratorScale::Element, typename FragmentScale::Element,
    ...
    if (gmem_zero_ptr != nullptr) {
      arch::global_load<FragmentScale, sizeof(FragmentScale)>(tb_frag_zeros, gmem_zero_ptr, iterator_scale.valid());
    }
