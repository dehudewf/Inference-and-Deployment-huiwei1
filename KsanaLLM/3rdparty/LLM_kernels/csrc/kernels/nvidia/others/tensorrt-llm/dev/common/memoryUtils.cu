...
cudaError_t cudaMemcpyAsyncSanitized(void* dst, void const* src, size_t count, enum cudaMemcpyKind kind,
                                     cudaStream_t stream) {
...
template void cudaAutoCpy(unsigned long long const** tgt, unsigned long long const* const* src, size_t size,
                          cudaStream_t stream);
...
template <typename T_IN, typename T_OUT>
void invokeCudaD2DScaleCpyConvert(T_OUT* tgt, const T_IN* src, float const* scale, bool invert_scale, const size_t size,
                                  cudaStream_t stream) {
...
void saveToBinary(T const* ptr, const size_t size, std::string filename) {
  ...
  KLLM_KERNEL_CHECK_WITH_INFO(out.is_open(), "Fail to open file " + filename);
...
template <typename T_OUT, typename T_IN>
__global__ void transpose0213(T_OUT* dst, T_IN* src, const size_t dim0, const size_t dim1, const size_t dim2,
                              const size_t dim3) {
...
template <typename T>
void invokeInPlaceTranspose0213(T* data, T* workspace, const size_t dim0, const size_t dim1, const size_t dim2,
                                const size_t dim3) {
  // copy data to workspace, and then transpose from workspace to data
...
template void invokeFakeCast<__nv_bfloat16, __nv_fp8_e4m3>(__nv_bfloat16* input_ptr, const size_t size,
                                                           cudaStream_t stream);
#endif
...
template bool invokeCheckRange<int>(int const* buffer, const size_t size, int min, int max, bool* d_within_range,
                                    cudaStream_t stream);
