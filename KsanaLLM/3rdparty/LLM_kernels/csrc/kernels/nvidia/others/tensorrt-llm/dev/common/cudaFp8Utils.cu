...
void invokeDequantizeMatrix(T_OUT* output, T_S const* input_scale, T_IN const* input, int64_t numel, int64_t lda,
  ...
  config.attrs = attrs;
  if (quantize_mode == QuantizeMode::PER_CHANNEL) {
    cudaLaunchKernelEx(&config, scaleMatrix<QuantizeMode::PER_CHANNEL, false, T_OUT, T_S, T_IN>, output, input_scale,
                       input, numel, lda);
  } else if (quantize_mode == QuantizeMode::PER_TOKEN) {
...
template void invokeFakeQuantize<__nv_fp8_e4m3, float, float>(float* dst, float const* src, const int64_t numel,
                                                              cudaStream_t stream);
template void invokeFakeQuantize<float, float, __nv_fp8_e4m3>(float* dst, __nv_fp8_e4m3 const* src, const int64_t numel,
                                                              cudaStream_t stream);
template void invokeFakeQuantize<__nv_fp8_e4m3, half, half>(half* dst, half const* src, const int64_t numel,
                                                            cudaStream_t stream);
template void invokeFakeQuantize<__nv_fp8_e4m3, __nv_bfloat16, __nv_bfloat16>(__nv_bfloat16* dst,
                                                                              __nv_bfloat16 const* src,
                                                                              const int64_t numel, cudaStream_t stream);

template void invokeFakeQuantize<float, half, float>(half* dst, float const* src, const int64_t numel,
                                                     cudaStream_t stream);
...
#  define DEFINE_INVOKE_QUANTIZE_MATRIX(type_out, type_scale, type_in)                                      \
    template void invokeQuantizeMatrix<type_out, type_scale, type_in>(                                      \
        type_out * output, type_scale const* input_scale, type_in const* input, int64_t numel, int64_t lda, \
        QuantizeMode quantize_mode, cudaStream_t stream);                                                   \
    ...
    template void invokeComputeScalesAndQuantizeMatrix<type_out, type_scale, type_in>(                      \
        type_out * output, type_scale * input_scale, type_in const* input, int64_t numel, int64_t lda,      \
        QuantizeMode quantize_mode, cudaStream_t stream);
