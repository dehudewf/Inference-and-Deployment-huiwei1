...
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_extensions/gemm/kernel/fused_moe_kernel.cuh"
...
void sm80_generic_fused_moe_gemm_kernelLauncher(ElementType_ const* A, CutlassWeightType_ const* B,
                                                ElementType_ const* biases, bool bias_is_broadcast, ElementType_* C,
                                                int64_t const* total_tokens_including_expert, int64_t num_rows,
  ...
  if (kernel_occupancy != nullptr) {
    ...
    llm_kernels::nvidia::tensorrt_llm::dev::common::check_cuda_error(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
  ...
  KLLM_KERNEL_CHECK_WITH_INFO(occupancy > 0, "GPU lacks the shared memory resources to run fused_moe kernel");
