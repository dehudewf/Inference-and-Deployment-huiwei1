...
#include <random>
#include <utility>
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/common/NvInferDataType.h"
...
struct ActivationParams {
  ...
  explicit ActivationParams(ActivationType activation_type) : activation_type(activation_type) {
    KLLM_KERNEL_CHECK_WITH_INFO(activation_type != ActivationType::SwigluBias,
...
struct MOEParallelismConfig {
  ...
  MOEParallelismConfig(int tp_size, int tp_rank, int ep_size, int ep_rank, int cluster_size, int cluster_rank)
    ...
    KLLM_KERNEL_CHECK(ep_rank >= 0);
    KLLM_KERNEL_CHECK(ep_size >= 1);
    KLLM_KERNEL_CHECK(cluster_rank < cluster_size);
    KLLM_KERNEL_CHECK(cluster_rank >= 0);
    KLLM_KERNEL_CHECK(cluster_size >= 1);
    KLLM_KERNEL_CHECK(ep_size == 1 || cluster_size == 1);
  }
...
class CutlassMoeFCRunnerInterface {
 public:
  ...
  virtual size_t getWorkspaceSize(int64_t const num_rows, int64_t const hidden_size, int64_t const inter_size,
                                  ...
                                  bool use_deepseek_fp8_block_scale, bool min_latency_mode, bool use_awq) = 0;
  ...
  virtual void gemm1(void const* const input, void* const output, void* const intermediate_result,
                     ...
                     TmaWarpSpecializedGroupedGemmInput tma_ws_input_template, void const* const fc1_expert_weights,
                     void const* const fc1_expert_biases, int64_t const* const num_valid_tokens_ptr,
                     void const* const fc1_int_scales, float const* const fc1_fp8_dequant,
                     ...
                     float const** alpha_scale_ptr_array, bool bias_is_broadcast, bool use_deepseek_fp8_block_scale,
  ...
  computeStridesTmaWarpSpecializedDispatch(
      ...
      int const* permuted_row_to_unpermuted_row, cudaStream_t stream) = 0;
...
class CutlassMoeFCRunner : public CutlassMoeFCRunnerInterface {
  using DeepSeekBlockScaleGemmRunner =
      llm_kernels::nvidia::tensorrt_llm::dev::kernels::fp8_blockscale_gemm::CutlassFp8BlockScaleGemmRunnerInterface;
 ...
 public:
  ...
  void gemm1(void const* const input, void* const output, void* const intermediate_result,
    ...
    return Self::gemm1(moe_gemm_runner_, block_scale_gemm_runner, static_cast<T const*>(input), static_cast<T*>(output),
                       ...
                       fc1_fp4_act_flat, fc2_fp4_act_flat, quant_params, num_rows, expanded_num_rows, hidden_size,
                       inter_size, num_experts_per_node, fc1_activation_type, alpha_scale_ptr_array, bias_is_broadcast,
                       stream, config, min_latency_mode, num_active_experts_per, active_expert_global_ids);
  }

  void gemm2(void const* const input, void* const gemm_output, void* const final_output,
             int64_t const* const expert_first_token_offset,
             TmaWarpSpecializedGroupedGemmInput const tma_ws_input_template, void const* const fc2_expert_weights,
             void const* const fc2_expert_biases, void const* const fc2_int_scales, float const* const fc2_fp8_dequant,
             TmaWarpSpecializedGroupedGemmInput::ElementSF const* fc2_fp4_act_flat, QuantParams quant_params,
             float const* const token_topk_unpermuted_scales, float const* const token_topk_permuted_scales,
             int const* const unpermuted_row_to_permuted_row, int const* permuted_row_to_unpermuted_row,
  ...
  computeStridesTmaWarpSpecializedDispatch(
    ...
    return Self::computeStridesTmaWarpSpecialized(
        expert_first_token_offset, layout_info1, layout_info2, num_tokens, expanded_num_tokens, gemm1_n, gemm1_k,
        gemm2_n, gemm2_k, num_experts_per_node, reinterpret_cast<T const*>(gemm1_in),
        reinterpret_cast<T const*>(gemm2_in), reinterpret_cast<WeightType const*>(weights1),
        reinterpret_cast<WeightType const*>(weights2), alpha_scale_flat1, alpha_scale_flat2, fp4_act_flat1,
        fp4_act_flat2, quant_params, reinterpret_cast<ScaleBiasType const*>(bias1),
        reinterpret_cast<ScaleBiasType const*>(bias2), reinterpret_cast<UnfusedGemmOutputType*>(gemm1_output),
        reinterpret_cast<UnfusedGemmOutputType*>(gemm2_output), router_scales, permuted_row_to_unpermuted_row, stream);
  }
  ...
  computeStridesTmaWarpSpecializedLowLatencyDispatch(
    ...
    return Self::computeStridesTmaWarpSpecializedLowLatency(
        ...
        reinterpret_cast<WeightType const*>(weights1), reinterpret_cast<WeightType const*>(weights2), fp8_dequant1,
        fp8_dequant2, fc1_fp4_act_flat, fc2_fp4_act_flat, quant_params, reinterpret_cast<ScaleBiasType const*>(bias1),
...
struct GemmProfilerBackend {
 public:
  ...
  void init(CutlassMoeFCRunnerInterface& runner, GemmToProfile gemm_to_profile, nvinfer1::DataType dtype,
            ...
            int64_t unpadded_hidden_size, int64_t inter_size, int64_t group_size, ActivationType activation_type,
            bool bias, bool use_lora, bool min_latency_mode, bool need_weights, MOEParallelismConfig parallelism_config,
