...
using namespace llm_kernels::nvidia::tensorrt_llm::dev::cutlass_extensions;
...
TileShape get_cta_shape_for_config(CutlassTileConfig tile_config) {
  switch (tile_config) {
    ...
    case CutlassTileConfig::CtaShape16x256x128_WarpShape16x64x128:
      return TileShape{16, 256};
    default:
      KLLM_KERNEL_THROW("[get_grid_shape_for_config] Invalid config");
  }
}
...
std::vector<CutlassGemmConfig> get_candidate_configs_sm90(CutlassGemmConfig::CandidateConfigTypeParam const config) {
  ...
  for (auto const& tile_config : tiles) {
    ...
    if (has_w4afp8) {
      ...
      for (auto const& mainloop_schedule : mainloop_schedules) {
        ...
        candidate =
            CutlassGemmConfig(tile_config, mainloop_schedule, epilogue_schedule, ClusterShape::ClusterShape_2x1x1);
        candidate_configs.push_back(candidate);
        candidate =
            CutlassGemmConfig(tile_config, mainloop_schedule, epilogue_schedule, ClusterShape::ClusterShape_1x2x1);
        candidate_configs.push_back(candidate);
        candidate =
            CutlassGemmConfig(tile_config, mainloop_schedule, epilogue_schedule, ClusterShape::ClusterShape_2x2x1);
        candidate_configs.push_back(candidate);
      }
    } else {
  ...
  if (config & CutlassGemmConfig::WEIGHT_ONLY) {
    if (tiles.size() > 0 && !(config & CutlassGemmConfig::GROUPED_GEMM)) {
      CutlassGemmConfig CudaKernelConfig(tiles[0], MainloopScheduleType::AUTO, EpilogueScheduleType::AUTO,
                                         ClusterShape::ClusterShape_1x1x1);
...
std::vector<CutlassGemmConfig> get_candidate_configs_sm100_dynamic_cluster_shape(
  ...
  if ((config & CutlassGemmConfig::FP4_ONLY) != 0) {
    ...
    candidate_configs.push_back(CutlassGemmConfig{CutlassTileConfigSM100::CtaShape128x64x128B,
                                                  MainloopScheduleType::AUTO, schedule, cluster1sm,
                                                  dynamic_cluster_shape, fallback_cluster_shape});
    ...
    if (supports_2sm) {
      candidate_configs.push_back(CutlassGemmConfig{CutlassTileConfigSM100::CtaShape128x64x128B,
                                                    MainloopScheduleType::AUTO, schedule, cluster2sm,
                                                    dynamic_cluster_shape, fallback_cluster_shape});
      candidate_configs.push_back(CutlassGemmConfig{CutlassTileConfigSM100::CtaShape128x128x128B,
                                                    MainloopScheduleType::AUTO, schedule, cluster2sm,
                                                    dynamic_cluster_shape, fallback_cluster_shape});
    ...
    for (auto schedule : {EpilogueScheduleType::TMA, EpilogueScheduleType::NO_SMEM}) {
      ...
      for (auto cluster_shape : cluster_shapes) {
        auto fallback_cluster_shape = cluster_shape == ClusterShape::ClusterShape_1x1x1
                                          ...
                                          : ClusterShape::ClusterShape_2x1x1;
        auto configs =
            get_candidate_configs_sm100_dynamic_cluster_shape(config, schedule, cluster_shape, fallback_cluster_shape);
        candidate_configs.insert(candidate_configs.end(), configs.begin(), configs.end());
      }

      auto configs = get_candidate_configs_sm100_dynamic_cluster_shape(config, schedule, ClusterShape::Undefined,
                                                                       ClusterShape::Undefined);
      candidate_configs.insert(candidate_configs.end(), configs.begin(), configs.end());
    }
    return candidate_configs;
  } else {
    KLLM_KERNEL_THROW("Not Implemented: SM100 GEMM candidates have not been defined.");
  }
#endif
}
...
std::vector<CutlassGemmConfig> get_candidate_configs_sm120(CutlassGemmConfig::CandidateConfigTypeParam const config) {
  ...
  if (config & CutlassGemmConfig::GROUPED_GEMM) {
    ...
    if ((config & CutlassGemmConfig::FP4_ONLY) != 0) {
      ...
      KLLM_KERNEL_THROW("Not Implemented: SM120 group GEMM only supports nvfp4.");
    }
  } else {
    std::vector<CutlassGemmConfig> candidate_configs;
    if ((config & CutlassGemmConfig::FP4_ONLY) != 0) {
      ...
      KLLM_KERNEL_THROW("Not Implemented: SM120 GEMM only supports nvfp4.");
    }
  }
#endif
...
CutlassGemmConfig estimate_best_config_from_occupancies(std::vector<CutlassGemmConfig> const& candidate_configs,
                                                        std::vector<int> const& occupancies, int64_t const m,
                                                        int64_t const n, int64_t const k, int64_t const num_experts,
                                                        int const split_k_limit, size_t const workspace_bytes,
    ...
    for (int split_k_factor = 1; split_k_factor <= max_split_k; ++split_k_factor) {
      if (is_valid_split_k_factor(m, n, k, tile_shape, split_k_factor, workspace_bytes, is_weight_only)) {
        ...
        if (current_score < config_score ||
          ...
          best_config = CutlassGemmConfig(candidate_config.tile_config_sm80, split_style, split_k_factor,
  ...
  if (best_config.tile_config_sm80 == CutlassTileConfig::ChooseWithHeuristic) {
    KLLM_KERNEL_THROW("Heuristic failed to find a valid config.");
  }
