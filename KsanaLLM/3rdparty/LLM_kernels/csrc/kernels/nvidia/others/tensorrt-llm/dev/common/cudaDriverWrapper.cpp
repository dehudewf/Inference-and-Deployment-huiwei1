...
#include <cuda.h>
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/common/cudaDriverWrapper.h"
...
CUDADriverWrapper::CUDADriverWrapper() : handle(dllOpen(CUDA_LIB_NAME)) {
  KLLM_KERNEL_CHECK_WITH_INFO(handle != nullptr, "CUDA driver library is not open correctly.");
...
CUresult CUDADriverWrapper::cuLinkDestroy(CUlinkState state) const { return (*_cuLinkDestroy)(state); }

CUresult CUDADriverWrapper::cuModuleLoadData(CUmodule* module, void const* image) const {
  return (*_cuModuleLoadData)(module, image);
}

CUresult CUDADriverWrapper::cuLinkCreate(unsigned int numOptions, CUjit_option* options, void** optionValues,
...
CUresult CUDADriverWrapper::cuLibraryGetGlobal(CUdeviceptr* dptr, size_t* bytes, CUlibrary library,
                                               char const* name) const {
  return (*_cuLibraryGetGlobal)(dptr, bytes, library, name);
}

CUresult CUDADriverWrapper::cuLibraryUnload(CUlibrary library) const { return (*_cuLibraryUnload)(library); }

CUresult CUDADriverWrapper::cuKernelSetAttribute(CUfunction_attribute attrib, int val, CUkernel kernel,
                                                 CUdevice dev) const {
  return (*_cuKernelSetAttribute)(attrib, val, kernel, dev);
}

CUresult CUDADriverWrapper::cuCtxGetDevice(CUdevice* device) const { return (*_cuCtxGetDevice)(device); }
...
CUresult CUDADriverWrapper::cuLaunchCooperativeKernel(CUfunction f, unsigned int gridDimX, unsigned int gridDimY,
  ...
  return (*_cuLaunchCooperativeKernel)(f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ, sharedMemBytes,
...
CUresult CUDADriverWrapper::cuLaunchKernel(CUfunction f, unsigned int gridDimX, unsigned int gridDimY,
  ...
  return (*_cuLaunchKernel)(f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ, sharedMemBytes, hStream,
                            kernelParams, extra);
}
...
CUresult CUDADriverWrapper::cuLaunchKernelEx(CUlaunchConfig const* config, CUfunction f, void** kernelParams,
                                             void** extra) const {
  TLLM_LOG_DEBUG("Launch config: %s", stringify_launch_config(*config).c_str());
  KLLM_KERNEL_CHECK_DEBUG_WITH_INFO((extra != nullptr) != (kernelParams != nullptr),
...
CUresult CUDADriverWrapper::cuOccupancyMaxActiveClusters(int* maxActiveClusters, CUfunction f,
