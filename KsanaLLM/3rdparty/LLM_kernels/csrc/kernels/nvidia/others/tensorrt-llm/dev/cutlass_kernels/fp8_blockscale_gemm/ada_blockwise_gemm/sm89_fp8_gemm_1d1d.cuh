...
CUTLASS_GLOBAL void sm89_fp8_bmm_1d1d_impl(uint32_t shape_m, uint32_t shape_n, uint32_t shape_k, __nv_fp8_e4m3* A,
                                           __nv_fp8_e4m3* B, __nv_bfloat16* D, float* scales_a, float* scales_b,
...
struct AdaBlockwiseGemmKernel {
  ...
  CUTE_DEVICE auto gmem_tensor_init(typename KT::ElementInput const* ptr_a, typename KT::ElementInput const* ptr_b,
                                    typename KT::ElementBlockScale const* ptr_scale_a,
                                    typename KT::ElementBlockScale const* ptr_scale_b, uint32_t M, uint32_t N,
                                    uint32_t K, int* SharedStorageBase) {
    ...
    auto mSFA_mk = cute::make_tensor(cute::make_gmem_ptr(ptr_scale_a), cute::make_shape(ScaleM, ScaleK),
                                     cute::make_stride(cute::_1{}, ScaleM));

    auto mSFB_nk = cute::make_tensor(cute::make_gmem_ptr(ptr_scale_b), cute::make_shape(ScaleN, ScaleK),
                                     cute::make_stride(ScaleK, cute::_1{}));
    ...
    auto gSFA = cute::local_tile(mSFA_mk, typename KT::ScalePerTileShape{}, cta_coord,
                                 cute::Step<_1, X, _1>{});  // (BLK_M,BLK_K)
    auto gSFB = cute::local_tile(mSFB_nk, typename KT::ScalePerTileShape{}, cta_coord,
                                 cute::Step<X, _1, _1>{});  // (BLK_N,BLK_K)
  ...
  CUTE_DEVICE void epilogue_with_smem(Accumulator& accum, SharedStorage& shared_storage, ElementOutput* o, int M,
                                      int N) {
    // convert type
  ...
  void operator()(typename KT::ElementInput const* ptr_a, typename KT::ElementInput const* ptr_b,
    ...
    auto tApA = cute::make_tensor<bool>(cute::make_shape(cute::size<1>(tAsA), cute::size<2>(tAsA)),
    ...
    auto tBpB = cute::make_tensor<bool>(cute::make_shape(cute::size<1>(tBsB), cute::size<2>(tBsB)),
                                        cute::Stride<cute::_1, cute::_0>{});
    CUTLASS_PRAGMA_UNROLL
    ...
    auto tApSFA = cute::make_tensor<bool>(cute::make_shape(cute::size<1>(tAsSFA), cute::size<2>(tAsSFA)),
                                          cute::Stride<cute::_1, cute::_0>{});
    CUTLASS_PRAGMA_UNROLL
    ...
    auto accum = cute::partition_fragment_C(mma, cute::make_shape(cute::Int<KT::kTileM>{}, cute::Int<KT::kMmaPermN>{},
    ...
    cute::for_each(cute::make_int_sequence<KT::NUM_GROUP_N>{}, [&](auto n_block) {
      ...
      cute::clear(temp);
      if constexpr (n_block == 0) {
        cute::for_each(cute::make_int_sequence<cute::size(scale)>{}, [&](auto i) { scale(i) = tXrSFA(i) * tXrSFB(0); });
      }
      cute::gemm(mma, tCrA, tCrB(cute::_, cute::_, cute::_, n_block), temp);
      promote(accum, temp, scale, n_block);
    });
