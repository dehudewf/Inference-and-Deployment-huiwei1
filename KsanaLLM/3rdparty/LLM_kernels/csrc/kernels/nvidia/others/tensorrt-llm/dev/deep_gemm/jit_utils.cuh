...
inline int get_smem_size(int num_stages, int k, int block_m, int block_n, int block_k = 128, bool swap_ab = false) {
  if (!swap_ab) {
...
inline GemmConfig get_best_gemm_config(uint32_t shape_m, uint32_t shape_n, uint32_t shape_k, int num_groups,
  ...
  auto get_last_wave_util = [shape_m, shape_n, num_groups, num_device_sms, &fix_wave_saturate](int block_m,
                                                                                               int block_n) -> int {
  ...
  for (int block_m : block_ms) {
    for (int block_n : block_ns) {
      ...
      if (best_block_m == 0 || best_block_n == 0) {
        ...
        success = util > best_util || (util == best_util &&
