...
struct CollectiveMmaInterleaved<MainloopSm90TmaGmmaRmemAWarpSpecializedMixedInput<Stages, ClusterShape, KernelSchedule>,
 ...
 public:
  ...
  using SmemLayoutA =
      decltype(tile_to_shape(InternalSmemLayoutAtomA{},
                             make_shape(shape<0>(TileShape{}), shape<2>(TileShape{}), Int<DispatchPolicy::Stages>{}),
                             cute::conditional_t<::cutlass::gemm::detail::is_major<0, InternalStrideA>(),
                                                 Step<_2, _1, _3>, Step<_1, _2, _3>>{}));

  using Layout_Interleave =
      decltype(cute::composition(SmemLayoutA{}.layout_a(), SmemLayoutA{}.offset(),
  ...
  using SmemLayoutA_mma_interleave = decltype(tile_to_shape(
      Layout_Interleave{}, make_shape(shape<0>(TileShape{}), shape<2>(TileShape{}), Int<DispatchPolicy::Stages>{}),
  ...
  using SmemLayoutB =
      decltype(tile_to_shape(InternalSmemLayoutAtomB{},
                             make_shape(shape<1>(TileShape{}), shape<2>(TileShape{}), Int<DispatchPolicy::Stages>{}),
                             cute::conditional_t<::cutlass::gemm::detail::is_major<0, InternalStrideB>(),
                                                 Step<_2, _1, _3>, Step<_1, _2, _3>>{}));

  // It is assumed that the scales and zero-points share the same smem layout
  using SmemLayoutScale = decltype(tile_to_shape(
      SmemLayoutAtomScale{}, make_shape(shape<0>(ScaleTileShape{}), shape<1>(ScaleTileShape{}), Int<Stages>{}),
 ...
 private:
  ...
  static constexpr uint32_t compute_tma_transaction_bytes_mk() {
    constexpr uint32_t baseline_bytes = cutlass::bits_to_bytes(
        size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) * static_cast<uint32_t>(cute::sizeof_bits_v<InternalElementA>));
 ...
 public:
  ...
  struct Params {
   private:
    using Outer =
        CollectiveMmaInterleaved<DispatchPolicy, TileShape_, ElementAOptionalTuple, StrideA_, ElementBOptionalTuple,
  ...
  static constexpr Params to_underlying_arguments(ProblemShape const& problem_shape, Arguments const& args,
    ...
    typename Params::TMA_A tma_load_a =
        make_tma_copy<TmaElementA>(GmemTiledCopyA{}, tensor_a, SmemLayoutA{}(_, _, cute::Int<0>{}),
    ...
    if constexpr (KernelConversionMode == ConversionMode::DirectConvert) {
      ...
      if constexpr (KernelConversionMode == ConversionMode::ConvertAndScale) {
        return {tma_load_a,      tma_load_b,          tma_load_scale,        tma_load_zero,        scale_k,
                args.group_size, TmaTransactionBytes, TmaTransactionBytesMK, TmaTransactionBytesNK};
      } else if constexpr (KernelConversionMode == ConversionMode::ConvertAndScaleWithZero) {
  ...
  CUTLASS_DEVICE void load(Params const& mainloop_params, MainloopPipeline pipeline, PipelineState smem_pipe_write,
                           cute::tuple<Ts...> const& load_inputs, BlockCoord const& blk_coord,
    ...
    if (lane_predicate) {
      ...
      auto extra_input_partitions = partition_extra_tma_inputs(mainloop_params, load_inputs, shared_tensors,
                                                               cluster_local_block_id, m_coord, l_coord);
      ...
      for (; k_tile_count > 0; --k_tile_count) {
        ...
        if constexpr (KernelConversionMode == ConversionMode::DirectConvert) {
          ...
          if constexpr (KernelConversionMode == ConversionMode::ConvertAndScale) {
            ...
            copy(mainloop_params.tma_load_zero.with(*tma_barrier, mcast_mask_s), tZgZ(_, _, _, scale_load_k),
  ...
  constexpr auto interleave_for_mixed_input() {
    if constexpr (cute::sizeof_bits_v<WeightType> == 4 && cute::sizeof_bits_v<ActivationType> == 8) {
      return Layout<Shape<Shape<_4, _2, _2>, _1, Shape<_2, _2>>, Stride<Stride<_1, _8, _4>, _0, Stride<_16, _32>>>{};
    } else if constexpr (cute::sizeof_bits_v<WeightType> == 4 && cute::sizeof_bits_v<ActivationType> == 16) {
      return Layout<Shape<Shape<_2, _2, _2, _2>, _1, Shape<_2>>, Stride<Stride<_1, _4, _8, _2>, _0, Stride<_16>>>{};
    } else if constexpr (cute::sizeof_bits_v<WeightType> == 8 && cute::sizeof_bits_v<ActivationType> == 16) {
      return Layout<Shape<Shape<_2, _2, _2>, _1, Shape<_2, _2>>, Stride<Stride<_1, _4, _2>, _0, Stride<_8, _16>>>{};
    } else {
      static_assert(dependent_false<WeightType, ActivationType>,
                    "unsupported weight and activation, must be one of w4a8,w4a16,w8a16");
    }
  ...
  CUTLASS_DEVICE void mma(MainloopPipeline pipeline, PipelineState smem_pipe_read, FrgTensorC& accum, int k_tile_count,
    ...
    Tensor sA_ =
        make_tensor(make_smem_ptr(shared_tensors.smem_A.begin()), SmemLayoutA_mma_interleave{});  // (BLK_M,BLK_K,PIPE)
    Tensor sA = as_position_independent_swizzle_tensor(sA_);                                      // (BLK_M,BLK_K,PIPE)
    ...
    static_assert(
        A_CPY_VEC_remapped{} == 32 / cutlass::sizeof_bits<InternalElementA>::value,
 ...
 private:
  ...
  CUTLASS_DEVICE auto retile_extra_mma_info(TiledMma const& tiled_mma, cute::tuple<Ts...>& partitioned_extra_info,
  ...
  CUTLASS_DEVICE void transform_A_kblock(TCrA_load const& tCrA_load, cute::Int<VectorWidthA> vec_A, TCrA_mma& tCrA_mma,
    ...
    if (kNumKIterationsPerWarpBLoad != 1) {
      if (k_block % kNumKIterationsPerWarpBLoad == 0) {
        ...
        if constexpr (KernelConversionMode == ConversionMode::DirectConvert) {
          transform_internal_A(tCrA_load_reshaped(_, _, k_block_load), vec_A, tCra_mma_reshaped(_, _, k_block_load));
        } else if constexpr (KernelConversionMode == ConversionMode::ConvertAndScale) {
          ...
          transform_internal_A(tCrA_load_reshaped(_, _, k_block_load), vec_A,
                               make_fragment_like<ElementScale>(tCra_mma_reshaped)(_, _, k_block_load),
                               tCrS_reshaped(_, _, 0), tCrZ_reshaped(_, _, 0), tCra_mma_reshaped(_, _, k_block_load));
        } else {
      ...
      if constexpr (KernelConversionMode == ConversionMode::DirectConvert) {
        ...
        auto tCrS = cute::get<1>(partitioned_extra_info);
        transform_internal_A(tCrA_load(_, _, k_block), vec_A, make_fragment_like<ElementScale>(tCrA_mma)(_, _, k_block),
  ...
  CUTLASS_DEVICE void convert_tensor(Tensor<EngineIn, TensorLayout> const& in, Tensor<EngineOut, TensorLayout>& out,
    ...
    using Converter = std::conditional_t < cutlass::sizeof_bits_v<SrcType>
  ...
  CUTLASS_DEVICE void transform_internal_A(Tensor<EngineIn, TensorLayout>&& in,
                                           ...
                                           Tensor<EngineInputBuffer, TensorLayout>&& converted_inputs,
                                           Tensor<EngineScale, TensorScaleLayout>&& scales,
                                           Tensor<EngineZero, TensorScaleLayout>&& zeros,
                                           Tensor<EngineOut, TensorLayout>&& out) {
    static_assert(cute::is_same_v<typename EngineInputBuffer::value_type, typename EngineScale::value_type>,
                  "Type of the engine input buffer must equal the scale buffer");
