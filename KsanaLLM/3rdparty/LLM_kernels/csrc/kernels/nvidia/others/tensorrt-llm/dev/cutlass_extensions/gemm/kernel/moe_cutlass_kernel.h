...
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_extensions/gemm/kernel/gemm_moe_problem_visitor.h"
#include "csrc/kernels/nvidia/others/tensorrt-llm/dev/cutlass_extensions/tile_interleaved_layout.h"
 ...
 public:
  ...
  struct Params {
    ...
    Params(Arguments const& args, void* workspace = nullptr, int tile_count = 0)
        : problem_visitor(args.total_tokens_including_expert, args.gemm_n, args.gemm_k, args.problem_count, workspace,
                          tile_count),
    ...
    void update(Arguments const& args, void* workspace = nullptr, int tile_count = 0) {
      problem_visitor = typename ProblemVisitor::Params(args.total_tokens_including_expert, args.gemm_n, args.gemm_k,
  ...
  void run_kernel_(Params const& params, SharedStorage& shared_storage) {
    ...
    while (problem_visitor.next_tile()) {
      ...
      ElementScale* ptr_Scale =
          use_dq_gemm<Mma>::value ? params.weight_scales + problem_idx * gemm_k / params.group_size * gemm_n : nullptr;
      ...
      if constexpr (use_dq_gemm<Mma>::value) {
        ...
        typename Mma::IteratorScale iterator_scale = initialize_scale<typename Mma::IteratorScale, Mma::QuantOp>(
            LayoutScaleZero(ldm_Scale), reinterpret_cast<typename Mma::IteratorScale::Pointer>(ptr_Scale),
            reinterpret_cast<typename Mma::IteratorScale::Pointer>(ptr_Zero), {scale_row_extent, problem_size.n()},
            thread_idx, tb_offset_scale, params.group_size);
      ...
      typename Epilogue::OutputTileIterator iterator_C(params_C, ptr_C, problem_size.mn(), thread_idx,
                                                       threadblock_offset.mn(), params.scatter_D_indices);
      ...
      if constexpr (platform::is_same<
                        ...
                        cutlass::epilogue::thread::LinearCombination<
                            ...
                            typename EpilogueOutputOp::ElementAccumulator, typename EpilogueOutputOp::ElementCompute,
                            EpilogueOutputOp::kScale, EpilogueOutputOp::kRound>>::value) {
...
#  else
    static_assert(false,
                  "Invalid architecture being compiled. Only Ampere+ supported in weight-only quantization kernels.");
#  endif
#else
    CUTLASS_NOT_IMPLEMENTED();
#endif
