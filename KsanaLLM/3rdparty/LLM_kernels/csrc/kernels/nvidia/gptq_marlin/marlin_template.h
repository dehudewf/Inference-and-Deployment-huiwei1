...
#include "csrc/kernels/nvidia/gptq_marlin/dequant.h"
...
template <typename scalar_t>
__device__ inline void mma_trans(const typename ScalarType<scalar_t>::FragA& a_frag,
                                 const typename ScalarType<scalar_t>::FragB& frag_b,
...
template <typename scalar_t>
__device__ inline void scale(typename ScalarType<scalar_t>::FragB& frag_b, typename ScalarType<scalar_t>::FragS& frag_s,
                             int i) {
...
__device__ inline void sub_zp(typename ScalarType<scalar_t>::FragB& frag_b,
                              typename ScalarType<scalar_t>::scalar_t2& frag_zp, int i) {
  using scalar_t2 = typename ScalarType<scalar_t>::scalar_t2;
...
__device__ inline void scale4(typename ScalarType<scalar_t>::FragB& frag_b,
                              ...
                              typename ScalarType<scalar_t>::FragS& frag_s_4, int i) {
  using scalar_t2 = typename ScalarType<scalar_t>::scalar_t2;
  scalar_t2 s_val_1_2;
...
template <typename scalar_t,                                              // compute dtype, half or nv_float16
          ...
          >
__global__ void Marlin(const int4* __restrict__ A,               // fp16 input matrix of shape mxk
  ...
  static constexpr auto w_type = llm_kernels::nvidia::vllm_dtype::ScalarType::from_id(w_type_id);
  constexpr bool has_zp =
      w_type == llm_kernels::nvidia::vllm_dtype::kU4 || w_type == llm_kernels::nvidia::vllm_dtype::kU8;
  constexpr bool is_int_type =
      w_type == llm_kernels::nvidia::vllm_dtype::kU4 || w_type == llm_kernels::nvidia::vllm_dtype::kU8 ||
      w_type == llm_kernels::nvidia::vllm_dtype::kU4B8 || w_type == llm_kernels::nvidia::vllm_dtype::kU8B128;
  // see comments of dequant.h for more details
  constexpr bool dequant_skip_flop = !is_int_type ||
                                     has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value ||
                                     has_zp && !is_zp_float && !(w_type == llm_kernels::nvidia::vllm_dtype::kU8);

  scalar_t2 global_scale;

  if constexpr (w_type == llm_kernels::nvidia::vllm_dtype::kFE2M1f) {
  ...
  constexpr int s_tb_groups =
      !has_act_order && group_blocks != -1 && group_blocks < thread_k_blocks
          ? thread_k_blocks / group_blocks / (w_type == llm_kernels::nvidia::vllm_dtype::kFE2M1f ? 2 : 1)
          : 1;
  ...
  if constexpr (!has_act_order) {
    if constexpr (group_blocks == -1) {
      ...
      s_gl_rd = s_gl_stride * ((thread_k_blocks * slice_row) / group_blocks) /
                    (w_type == llm_kernels::nvidia::vllm_dtype::kFE2M1f ? 2 : 1) +
  ...
  int s_sh_rd;
  if constexpr (group_blocks != -1 && w_type == llm_kernels::nvidia::vllm_dtype::kFE2M1f) {
...
#pragma unroll
      for (int i = 0; i < a_sh_wr_iters; i++) {
        cp_async4_pred(&sh_a_stage[a_sh_wr_trans[i]], &A[a_gl_rd_delta_i * i + a_gl_rd + a_gl_rd_delta_o * a_off],
                       a_sh_wr_pred[i]);
      }
    ...
    if constexpr (!has_act_order) {
      ...
      if constexpr (group_blocks == -1) {
        ...
        if constexpr (group_blocks >= thread_k_blocks) {
          ...
          int cur_group_id = k_blocks / (group_blocks * (w_type == llm_kernels::nvidia::vllm_dtype::kFE2M1f ? 2 : 1));

          int4* sh_s_stage = sh_s + s_sh_stage * pipe;

          if constexpr (w_type_id != llm_kernels::nvidia::vllm_dtype::kFE2M1f.id()) {
...
#pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      if constexpr (has_act_order) {
        ...
        scale4<scalar_t>(frag_b0, act_frag_s[k2][0][j], act_frag_s[k2][1][j], act_frag_s[k2][2][j],
                         act_frag_s[k2][3][j], 0);
        scale4<scalar_t>(frag_b1, act_frag_s[k2][0][j], act_frag_s[k2][1][j], act_frag_s[k2][2][j],
                         act_frag_s[k2][3][j], 1);
      } else if constexpr (!dequant_skip_flop && has_zp && !is_zp_float && group_blocks == -1) {
        int idx = (threadIdx.x / 4) % 2;
        scalar_t2 s2 = Dtype::nums2num2(reinterpret_cast<scalar_t*>(&frag_s[j / 2][j % 2 * 2 + 0])[idx],
