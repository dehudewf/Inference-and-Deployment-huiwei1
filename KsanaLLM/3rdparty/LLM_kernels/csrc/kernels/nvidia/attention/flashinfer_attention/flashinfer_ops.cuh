 ...
 *
 * Adapted from
 * https://github.com/flashinfer-ai/flashinfer/blob/9220fb3443b5a5d274f00ca5552f798e225239b7/src/flashinfer_ops.cuh
 */
...
class BatchPrefillHandler {
 public:
  ...
  BatchPrefillHandler(void* page_locked_buffer, cudaStream_t& stream, bool enable_cuda_graph = false)
      : enable_cuda_graph_(enable_cuda_graph), stream_(stream) {
    page_locked_buffer_ = page_locked_buffer;
  }
  ~BatchPrefillHandler() {}
...
cudaError_t BatchPrefillWithPagedKVCacheWrapper(
    ...
    float rope_scale = 1.f, float rope_theta = 1e4, float* alibi_slopes_ptr = nullptr, cudaStream_t stream = nullptr) {
          ...
          {DISPATCH_pos_encoding_mode(
              ...
              {DISPATCH_use_fp16_qk_reduction(use_fp16_qk_reduction, USE_FP16_QK_REDUCTION, {
                ...
                Params params(q, paged_kv, /*custom_mask=*/nullptr, qo_indptr,
                              ...
                              /*alibi_slopes=*/alibi_slopes_ptr, num_qo_heads,
                              /*q_stride_n*/ (num_qo_heads + num_kv_heads * 2) * HEAD_DIM, /*q_stride_h*/ HEAD_DIM,
...
// indptr_h: (batch_size + 1), last_page_len_h: (batch_size) batch_size: number of sequences  
template <typename DTypeQ, typename DTypeKV, typename DTypeO, typename IdType>
