...
#include <csrc/kernels/nvidia/asymmetric_gemm/moe_gemm/launchers/fused_moe_gemm_launcher_sm80.h>
#include "csrc/kernels/nvidia/asymmetric_gemm/moe_gemm/launchers/moe_gemm_launcher_sm90.h"
#include "csrc/kernels/nvidia/asymmetric_gemm/moe_gemm/moe_gemm_kernels.h"
#include "csrc/kernels/nvidia/asymmetric_gemm/moe_gemm/moe_sm90_traits.h"
...
void genericMoeGemmKernelLauncher(T const* A, WeightType const* B, GemmOutputType const* weight_scales,
                                  ...
                                  int64_t const* total_tokens_including_expert, int64_t num_rows, int64_t gemm_n,
                                  int64_t gemm_k, int num_experts, cutlass_extensions::CutlassGemmConfig gemm_config,
                                  int const multi_processor_count, bool use_fused_moe,
  ...
  if (!use_fused_moe) {
    ...
    using EpilogueOp = typename llm_kernels::nvidia::cutlass_extensions::Epilogue<
    ...
    if constexpr ((std::is_same_v<T, __nv_fp8_e4m3> || std::is_same_v<T, __nv_fp8_e5m2>) &&
      ...
      KLLM_KERNEL_CHECK_WITH_INFO(
          weight_scales == nullptr && biases == nullptr && alpha_scale_ptr_array,
    ...
    if (kernel_occupancy != nullptr) {
      *kernel_occupancy = llm_kernels::nvidia::cutlass_extensions::compute_occupancy_for_kernel<GemmKernel>();
      return;
    }
    ...
    typename GemmGrouped::Arguments args(
        ...
        reinterpret_cast<CutlassWeightType const*>(B), reinterpret_cast<CutlassGemmOutputType const*>(weight_scales),
...
static void dispatch(T const* A, WeightType const* B, GemmOutputType const* weight_scales, GemmOutputType const* biases,
                     bool bias_is_broadcast, GemmOutputType* C, int64_t const* total_tokens_including_expert,
                     int64_t num_rows, int64_t gemm_n, int64_t gemm_k, int num_experts,
  ...
  if constexpr ((Stages == 2 || Arch::kMinComputeCapability >= 80) &&
    ...
    genericMoeGemmKernelLauncher<T, WeightType, GemmOutputType, Arch, EpilogueTag, ThreadblockShape, WarpShape, Stages>(
        A, B, weight_scales, biases, bias_is_broadcast, C, total_tokens_including_expert, num_rows, gemm_n, gemm_k,
        num_experts, gemm_config, multi_processor_count, use_fused_moe, alpha_scale_ptr_array, stream, occupancy);
  } else {
    KLLM_KERNEL_THROW("Cutlass gemm. Not instantiated for arch %d with stages set to %d", Arch::kMinComputeCapability,
                      Stages);
  }
}
...
void dispatchGemmConfig(T const* A, WeightType const* B, GemmOutputType const* weight_scales,
                        GemmOutputType const* biases, bool bias_is_broadcast, GemmOutputType* C,
                        int64_t const* total_tokens_including_expert, int64_t num_rows, int64_t gemm_n, int64_t gemm_k,
  ...
  switch (gemm_config.stages) {
    ...
    case 4:
      ...
      break;
    default:
      KLLM_KERNEL_THROW("dispatchGemmConfig does not support stages %d", gemm_config.stages);
      break;
  }
}
...
void dispatchMoeGemmToCutlass(T const* A, WeightType const* B, GemmOutputType const* weight_scales,
                              GemmOutputType const* biases, bool bias_is_broadcast, GemmOutputType* C,
                              int64_t const* total_tokens_including_expert, int64_t total_rows, int64_t gemm_n,
                              int64_t gemm_k, int num_experts, cutlass_extensions::CutlassGemmConfig gemm_config,
                              int multi_processor_count, bool use_fused_moe, float const** alpha_scale_ptr_array,
  ...
  switch (gemm_config.tile_config) {
    case cutlass_extensions::CutlassTileConfig::CtaShape16x128x64_WarpShape16x32x64:
      ...
      if constexpr (arch::kMinComputeCapability >= 75) {
        dispatchGemmConfig<T, WeightType, GemmOutputType, arch, EpilogueTag, cutlass::gemm::GemmShape<16, 128, 64>,
            ...
            A, B, weight_scales, biases, bias_is_broadcast, C, total_tokens_including_expert, total_rows, gemm_n,
    ...
    case cutlass_extensions::CutlassTileConfig::CtaShape32x128x64_WarpShape32x32x64:
      dispatchGemmConfig<T, WeightType, GemmOutputType, arch, EpilogueTag, cutlass::gemm::GemmShape<32, 128, 64>,
          ...
          A, B, weight_scales, biases, bias_is_broadcast, C, total_tokens_including_expert, total_rows, gemm_n, gemm_k,
          num_experts, gemm_config, multi_processor_count, use_fused_moe, alpha_scale_ptr_array, stream, occupancy);
      break;
...
MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::getAmpereConfigs(int sm) {
  using llm_kernels::nvidia::cutlass_extensions::CutlassGemmConfig;
  ...
  if (!isValidAmpereMOESpecialisation<T, WeightType>()) {
    return {};
  }
...
MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::getHopperConfigs(int sm) {
  ...
  if (!isValidHopperMOESpecialisation<T, WeightType>()) {
    return {};
  }
...
bool MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::supportsHopperSpecialisation() const {
  return sm_ == 90 && isValidHopperMOESpecialisation<T, WeightType>();
}
...
template <typename T, typename WeightType, typename OutputType, typename ScaleBiasType>
bool MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::supportsFusedGatedActivation(bool is_gated_activation,
                                                                                           int gemm_n,
                                                                                           int gemm_k) const {
...
void MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::dispatchToArch<EpilogueTag>(
    T const* A, WeightType const* B, ScaleBiasType const* weight_scales, ScaleBiasType const* biases,
    bool bias_is_broadcast, void* C_void, int64_t const* total_tokens_including_expert,
  ...
  KLLM_KERNEL_CHECK_WITH_INFO(sm_ >= 89 || !hopper_input.isValid(),
                              "Hopper input information is set for non specialised implementation");
  KLLM_KERNEL_CHECK_WITH_INFO(sm_ == 90 || !gemm_config.is_sm90,
                              "Hopper configuration provided for non-Hopper architecture");

  if (sm_ >= 70 && sm_ < 75) {
    dispatchMoeGemmToCutlass<T, WeightType, ScaleBiasType, cutlass::arch::Sm70, EpilogueTag>(
        A, B, weight_scales, biases, bias_is_broadcast, C, total_tokens_including_expert, total_rows, gemm_n, gemm_k,
        num_experts, gemm_config, multi_processor_count_, use_fused_moe, alpha_scale_ptr_array, stream, occupancy);
    ...
    if constexpr (isValidHopperMOESpecialisation<T, WeightType, EpilogueTag>()) {
      ...
      if (gemm_config.is_sm90) {
        KLLM_KERNEL_CHECK_WITH_INFO(biases != nullptr || hopper_input.ptr_c == nullptr,
                                    "Input biases and hopper input disagree if bias is enabled");
        KLLM_KERNEL_CHECK_WITH_INFO(hopper_input.isValid(), "Calling SM90 configuration with invalid hopper config");
        ...
        auto select_function = [&]() {
          switch (hopper_input.fusion) {
            ...
            default:
              KLLM_KERNEL_THROW("Unimplemented fusion %d requested", (int)hopper_input.fusion);
          };
        };
        auto selected_func = select_function();
        selected_func(hopper_input, num_experts, gemm_config, multi_processor_count_, stream, occupancy, nullptr);
        return;
      }
    ...
    if constexpr (isValidAmpereMOESpecialisation<T, WeightType, EpilogueTag>()) {
      KLLM_KERNEL_CHECK_WITH_INFO(
          !hopper_input.isValid(),
...
size_t MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::calcMaxWorkspaceSize(int num_experts) const {
  ...
  if constexpr (isValidHopperMOESpecialisation<T, WeightType>()) {
...
#define CALC_SIZE_FUSION(FUSION)                                                                                  \
  do {                                                                                                            \
    try {                                                                                                         \
      size_t size =                                                                                               \
          calcMaxWorkspaceSizeSM90<T, WeightType, OutputType, FUSION>(num_experts, conf, multi_processor_count_); \
      max_size = std::max(max_size, size);                                                                        \
      has_config = true;                                                                                          \
    } catch (llm_kernels::utils::KllmException const& e) {                                                        \
    }                                                                                                             \
  } while (0)
...
void MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::runGemm(
  ...
  dispatchToArch<EpilogueTag>(A, B, weight_scales, biases, bias_is_broadcast, C, total_tokens_including_expert,
                              hopper_input, total_rows, gemm_n, gemm_k, num_experts, chosen_conf, use_fused_moe,
                              alpha_scale_ptr_array, stream, nullptr);
}
  ...
  switch (activation_type) {
    case ActivationType::Relu:
      runGemm<cutlass_extensions::EpilogueOpDefaultReLU>(
          A, B, weight_scales, biases, bias_is_broadcast, C, total_tokens_including_expert, hopper_input, total_rows,
          gemm_n, gemm_k, num_experts, use_fused_moe, alpha_scale_ptr_array, stream, chosen_conf);
      break;
...
void MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::moeGemm(
    T const* A, WeightType const* B, ScaleBiasType const* weight_scales, void* C,
  ...
  runGemm<cutlass_extensions::EpilogueOpDefault>(A, B, weight_scales, nullptr, true, C, total_tokens_including_expert,
                                                 hopper_input, total_rows, gemm_n, gemm_k, num_experts, use_fused_moe,
