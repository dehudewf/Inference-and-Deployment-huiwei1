...
#include "csrc/kernels/nvidia/cutlass_extensions/epilogue/collective/epilogue_moe_finalize.hpp"
...
struct HopperGroupedGemmInfo
{
    ...
    static_assert(cutlass::platform::is_same<EpilogueTag, llm_kernels::nvidia::cutlass_extensions::EpilogueOpDefault>::value,
    ...
    int const multi_processor_count, cudaStream_t stream, int* kernel_occupancy, size_t* workspace_size)
{
    ...
    if constexpr (!should_filter_sm90_gemm_problem_shape_v<TileShape, ClusterShape, T>)
    {
        ...
        {
            *kernel_occupancy = llm_kernels::nvidia::cutlass_extensions::compute_occupancy_for_kernel<GemmKernel, true>();
            return;
        }
        ...
        KLLM_KERNEL_CHECK(hopper_input.stride_a);
        KLLM_KERNEL_CHECK(hopper_input.stride_b);
        KLLM_KERNEL_CHECK(hopper_input.ptr_a);
        KLLM_KERNEL_CHECK(hopper_input.ptr_b);
        ...
        KLLM_KERNEL_CHECK_WITH_INFO(calculated_ws_size <= hopper_input.gemm_workspace_size,
            "Workspace is size %zu but only %zu were allocated", calculated_ws_size, hopper_input.gemm_workspace_size);

        auto can_implement = gemm.can_implement(args);
        KLLM_KERNEL_CHECK_WITH_INFO(can_implement == cutlass::Status::kSuccess,
        ...
        KLLM_KERNEL_CHECK_WITH_INFO(init_status == cutlass::Status::kSuccess,
            "Failed to initialize cutlass SM90 grouped gemm. Error: "
                + std::string(cutlassGetStatusString(init_status)));

        auto run_status = gemm.run(stream);
        KLLM_KERNEL_CHECK_WITH_INFO(run_status == cutlass::Status::kSuccess,
            "Failed to run cutlass SM90 grouped gemm. Error: " + std::string(cutlassGetStatusString(run_status)));
        sync_check_cuda_error();
    }
    else
    {
        KLLM_KERNEL_THROW("Configuration was disabled by FAST_BUILD");
    }
