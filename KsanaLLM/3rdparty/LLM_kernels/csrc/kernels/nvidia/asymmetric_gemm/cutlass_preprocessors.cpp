...
LayoutDetails getLayoutDetailsForArch(QuantType quant_type) {
  ...
  switch (quant_type) {
    ...
    case QuantType::W4_AFP8:
      ...
      break;
    default:
      KLLM_KERNEL_THROW("Unsupported quantization type");
  }
  return details;
}
...
LayoutDetails getLayoutDetailsForTransform(QuantType quant_type, int arch) {
  if (arch >= 70 && arch < 75) {
    ...
    KLLM_KERNEL_CHECK_WITH_INFO(false, "Unsupported Arch");
...
std::vector<int> get_permutation_map(QuantType quant_type) {
  if (quant_type == QuantType::W8_A16) {
    ...
    return {0, 1, 8,  9,  16, 17, 24, 25, 2, 3, 10, 11, 18, 19, 26, 27,
            4, 5, 12, 13, 20, 21, 28, 29, 6, 7, 14, 15, 22, 23, 30, 31};
  } else if (quant_type == QuantType::W4_AFP8) {
    return {0, 1, 2,  3,  16, 17, 18, 19, 4,  5,  6,  7,  20, 21, 22, 23,
            8, 9, 10, 11, 24, 25, 26, 27, 12, 13, 14, 15, 28, 29, 30, 31};
  } else {
    KLLM_KERNEL_THROW("Invalid quantization type for LDSM permutation");
  }
}
...
void subbyte_transpose_impl(int8_t* transposed_quantized_tensor, int8_t const* quantized_tensor,
    ...
    for (size_t row_tile_start = 0; row_tile_start < num_rows; row_tile_start += M_TILE_L1) {
      for (size_t col_tile_start_byte = 0; col_tile_start_byte < col_bytes; col_tile_start_byte += N_TILE_L1) {
        ...
        if constexpr (bits_per_elt == 8) {
          ...
          KLLM_KERNEL_CHECK_WITH_INFO(false, "Unsupported quantization type.");
        }
...
void symmetric_quantize(int8_t* processed_quantized_weight, int8_t* unprocessed_quantized_weight,
  ...
  KLLM_KERNEL_CHECK_WITH_INFO(processed_quantized_weight, "Processed quantized tensor is NULL");
  KLLM_KERNEL_CHECK_WITH_INFO(scale_ptr, "Scale output pointer is NULL");
  KLLM_KERNEL_CHECK_WITH_INFO(input_weight_ptr, "Input weight pointer is NULL");
...
template void symmetric_quantize<half, float>(int8_t*, int8_t*, half*, float const*, std::vector<size_t> const&,
                                              QuantType, bool);
...
template void symmetric_quantize<float, float>(int8_t*, float*, float const*, std::vector<size_t> const&, QuantType,
                                               bool);

template void symmetric_quantize<half, float>(int8_t*, half*, float const*, std::vector<size_t> const&, QuantType,
                                              bool);
...
template void symmetric_quantize<__nv_bfloat16, half>(int8_t*, __nv_bfloat16*, half const*, std::vector<size_t> const&,
                                                      QuantType, bool);

template void symmetric_quantize<half, __nv_bfloat16>(int8_t*, half*, __nv_bfloat16 const*, std::vector<size_t> const&,
                                                      QuantType, bool);
