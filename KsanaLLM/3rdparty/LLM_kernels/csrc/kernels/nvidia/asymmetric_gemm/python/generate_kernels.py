...
class KLlm_EpilogueTag(enum.Enum):
...
class KLlm_EpilogueFusion(enum.Enum):
    epilogue_fusion_none = enum_auto()
    epilogue_fusion_finalize = enum_auto()


EpiTagNames = {
    KLlm_EpilogueTag.epilogue_op_default: "lc",  # linear combination
    KLlm_EpilogueTag.epilogue_op_bias:
    "lc_bias",  # linear combination with bias addition
    KLlm_EpilogueTag.epilogue_op_silu: "silu",  # silu or swiglu
    KLlm_EpilogueTag.epilogue_op_gelu: "gelu"  # gelu or geglu
}

EpiTag = {
    KLlm_EpilogueTag.epilogue_op_default:
    "llm_kernels::nvidia::cutlass_extensions::EpilogueOpDefault",
    KLlm_EpilogueTag.epilogue_op_bias:
    "llm_kernels::nvidia::cutlass_extensions::EpilogueOpBias",
    KLlm_EpilogueTag.epilogue_op_silu:
    "llm_kernels::nvidia::cutlass_extensions::EpilogueOpDefaultSilu",
    KLlm_EpilogueTag.epilogue_op_gelu:
    "llm_kernels::nvidia::cutlass_extensions::EpilogueOpDefaultFtGelu"
}

EpiFusion = {
    KLlm_EpilogueFusion.epilogue_fusion_none:
    "llm_kernels::nvidia::HopperGroupedGemmInput::EpilogueFusion::NONE",
    KLlm_EpilogueFusion.epilogue_fusion_finalize:
    "llm_kernels::nvidia::HopperGroupedGemmInput::EpilogueFusion::FINALIZE",
}

EpiFusionSuffixes = {
    None: "",
    KLlm_EpilogueFusion.epilogue_fusion_none: "EpilogueFusion_NONE",
    KLlm_EpilogueFusion.epilogue_fusion_finalize: "EpilogueFusion_FINALIZE",
}


################################################################################
# Quantization Operation and string utils
class KLlm_QuantOp(enum.Enum):
...
QuantOpNames = {
    KLlm_QuantOp.per_column_scale_only: "cs",
    KLlm_QuantOp.finegrained_scale_only: "fgs",
    KLlm_QuantOp.finegrained_scale_and_zeros: "fgsz",
    KLlm_QuantOp.none: "noquant"
}

QuantOpTag = {
    KLlm_QuantOp.per_column_scale_only:
    "cutlass::WeightOnlyQuantOp::PER_COLUMN_SCALE_ONLY",
    KLlm_QuantOp.finegrained_scale_only:
    "cutlass::WeightOnlyQuantOp::FINEGRAINED_SCALE_ONLY",
    KLlm_QuantOp.finegrained_scale_and_zeros:
    "cutlass::WeightOnlyQuantOp::FINEGRAINED_SCALE_AND_ZEROS",
    KLlm_QuantOp.none: "void"
}
...
{out_tag}*, int, int, int, const int, llm_kernels::nvidia::cutlass_extensions::CutlassGemmConfig, char*, size_t, cudaStream_t, int*
);
"""
...
def is_grouped_gemm_op_valid(op):
    ...
    if op.epi_tag != KLlm_EpilogueTag.epilogue_op_default:
        return False
...
def generate_sm90_mixed_gemm_operations():
    ...
    epi_tags = [KLlm_EpilogueTag.epilogue_op_bias]
    ...
    for dtype_combo, quant_op, epi_tag, cta_shape_mn, cga_shape in partial_args:
        ...
        fpA_intB_operation = KLlm_GemmLauncher(GemmKind.Gemm, arch, *dtype_combo, quant_op, epi_tag, cta_shape_mnk, \
...
def generate_sm90_grouped_gemm_operations():
    ...
    quant_ops = [KLlm_QuantOp.none]
    epi_tags = [KLlm_EpilogueTag.epilogue_op_default]
    ...
    for dtype, quant_op, epi_tag, epi_fusion, cta_shape_mn, cga_shape in partial_args:
        ...
        for otype in otypes:
            moe_gemm_operation = KLlm_GemmLauncher(
...
def generate_sm80_fused_grouped_gemm_operations():
    ...
    epi_tags = [
        KLlm_EpilogueTag.epilogue_op_silu, KLlm_EpilogueTag.epilogue_op_gelu
    ]
...
if __name__ == "__main__":
    ...
    fpA_intB_inl = "csrc/kernels/nvidia/asymmetric_gemm/fpA_intB_gemm/launchers/fpA_intB_launcher_sm90.inl"
    moe_gemm_inl = "csrc/kernels/nvidia/asymmetric_gemm/moe_gemm/launchers/moe_gemm_launcher_sm90.inl"
    sm80_moe_gemm_inl = "csrc/kernels/nvidia/asymmetric_gemm/moe_gemm/launchers/fused_moe_gemm_launcher_sm80.inl"
