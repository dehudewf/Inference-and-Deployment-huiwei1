...
#pragma once
#include "csrc/kernels/nvidia/weight_only_batched_gemv/common.h"
#include "csrc/kernels/nvidia/weight_only_batched_gemv/details.h"
...
__device__ __forceinline__ T warp_reduce_sum(T& val) {
  ...
  if (Interleave != 2 && Interleave != 4) val += __shfl_xor_sync(~0, val, 4);
  if (Interleave != 4) val += __shfl_xor_sync(~0, val, 2);
    ...
    for (int n = 0; n < CtaN; ++n) {
      ...
      if (lane_id < Interleave * ThreadsPerInterleavedTile && lane_id % ThreadsPerInterleavedTile == 0) {
        shmem[warp_id * CtaM * CtaN * Interleave + m * CtaN * Interleave + n * Interleave +
              lane_id / ThreadsPerInterleavedTile] = v;
      }
    }
  }
  __syncthreads();
