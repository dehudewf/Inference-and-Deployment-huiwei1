...
#include "csrc/kernels/nvidia/machete/cutlass_extensions/epilogue/broadcast_load_epilogue_c3x.hpp"
...
struct ScaledEpilogueBase {
 protected:
  ...
  template <typename Descriptor, typename T>
  static auto args_from_split_tensor(void* tensor_ptr, int64_t tensor_numel) {
    using Arguments = typename Descriptor::Arguments;
    auto* data_ptr = static_cast<T*>(tensor_ptr);
    if constexpr (std::is_same_v<Descriptor, ColOrScalarLoad<T>> ||
                  std::is_same_v<Descriptor, RowOrScalarLoad<T>>) {
      return Arguments{data_ptr, tensor_numel != 1};
    } else {
      static_assert(!std::is_same_v<Descriptor, ColLoad<T, true>> &&
                    !std::is_same_v<Descriptor, RowLoad<T, true>>);
      return Arguments{data_ptr};
...
struct ScaledEpilogue
 ...
 public:
  ...
  static ArgumentType prepare_args(void* a_scales_ptr, const int64_t a_scales_numel,
                                   void* b_scales_ptr, const int64_t b_scales_numel) {
    auto a_args = SUPER::template args_from_split_tensor<ScaleA, float>(a_scales_ptr, a_scales_numel);
    auto b_args = SUPER::template args_from_split_tensor<ScaleB, float>(b_scales_ptr, b_scales_numel);
