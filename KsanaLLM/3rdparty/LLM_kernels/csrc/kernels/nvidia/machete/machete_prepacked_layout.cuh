 ...
 * Adapted from
 * [vLLM Project]
 * https://github.com/vllm-project/vllm/blob/v0.8.2/csrc/quantization/machete/machete_prepacked_layout.cuh
 */
...
template <typename ElementA_, typename ElementB_, typename ElementConvert_, typename AccumulatorT, class LayoutB,
          class KernelSchedule, typename IlvBlkLayout_ = IlvBlkLayoutAuto>
// clang-format on
struct PrepackedLayoutBTemplate {
  ...
  using IlvdBlkLayout = std::conditional_t<
      ...
      std::conditional_t<should_interleave,
                         decltype(get_interleaved_blk_layout<ElementB, sizeof_bits_v<ElementConvert_>, 32>()), void>,
      IlvBlkLayout_>;
  ...
  using AtomLayoutMNK = cute::conditional_t<cute::is_same_v<KernelSchedule, KernelTmaWarpSpecializedCooperative>,
  ...
  CUTE_HOST_DEVICE static constexpr auto ppblock_ilvd_TV_to_offset() {
    ...
    if constexpr (std::is_same_v<IlvdBlkLayout, void>) {
      ...
      auto ilvd_FrgV = make_layout(make_shape(shape(ilvdBlk), Int<size(frgV) / size(ilvdBlk)>{}),
