 ...
 * Adapted from
 * [vLLM Project]
 * https://github.com/vllm-project/vllm/blob/65334ef3b9e4fd32ebc5c4e512debc25d5025488/csrc/core/scalar_type.hpp
 */
...
class ScalarType {
 public:
  ...
  constexpr ScalarType(uint8_t exponent, uint8_t mantissa, bool signed_, int32_t bias, bool finite_values_only = false,
  ...
  static constexpr ScalarType float_IEEE754(uint8_t exponent, uint8_t mantissa) {
    KLLM_KERNEL_CHECK(mantissa > 0 && exponent > 0);
  ...
  static constexpr ScalarType float_(uint8_t exponent, uint8_t mantissa, bool finite_values_only, NanRepr nan_repr) {
    KLLM_KERNEL_CHECK_WITH_INFO(nan_repr < NAN_REPR_ID_MAX, "Invalid NanRepr");
    KLLM_KERNEL_CHECK(mantissa > 0 && exponent > 0);
    KLLM_KERNEL_CHECK_WITH_INFO(nan_repr != NAN_IEEE_754,
 ...
 public:
  ...
  constexpr Id id() const {
    static_assert(id_size_bits() <= static_cast<int>(sizeof(Id) * 8), "ScalarType id is too large to be stored");
  ...
  constexpr int64_t size_bits() const { return mantissa + exponent + is_signed(); }
  ...
  constexpr bool has_nans() const { return is_floating_point() && nan_repr != NAN_NONE; }
  constexpr bool has_infs() const { return is_floating_point() && finite_values_only == false; }
  constexpr bool has_bias() const { return bias != 0; }

 private:
  double _floating_point_max() const {
    KLLM_KERNEL_CHECK_WITH_INFO(mantissa <= 52 && exponent <= 11, "Cannot represent max/min as a double for type ");
    ...
    if (nan_repr == NAN_EXTD_RANGE_MAX_MIN || nan_repr == NAN_NONE) {
      KLLM_KERNEL_CHECK_WITH_INFO(exponent < 11, "Cannot represent max/min as a double for type ");
      max_exponent += 1;
    }
  ...
  constexpr std::variant<int64_t, double> _raw_max() const {
    if (is_floating_point()) {
      ...
      KLLM_KERNEL_CHECK_WITH_INFO(size_bits() < 64 || (size_bits() == 64 && is_signed()),
  ...
  constexpr std::variant<int64_t, double> _raw_min() const {
    if (is_floating_point()) {
      KLLM_KERNEL_CHECK_WITH_INFO(is_signed(), "We currently assume all floating point types are signed");
...
static inline constexpr auto kFE8M23 = ScalarType::float_IEEE754(8, 23);
...
// colloquial names
static inline constexpr auto kFloat = kFE8M23;
